...
| Goofspiel | CPU/GPU simulator adapters, masks, renderer and CPU/CUDA standalone exact evaluators ported and tested. GPU parity covers 65,536 game transitions including exact-table refresh after graph capture, policy rows, non-default streams and recreation. The independent GitHub checkout at `2e39262cc` builds the ordinary GPU trainer. CPU/GPU exact-response refresh/persistence is implemented using checkpoint/configure hooks plus an optional three-line checkpoint-load callback; eight native continuation tests cover sync/async, graphs off/on, reservoir saturation, byte-identical restore and post-training evaluation. Latest/best exploitability sweeps use upstream metrics without new core code; four two-trial tests match returned scores to standalone checkpoint evaluations, with/without exact-response opponents. CUDA exact values match CPU within 5e-7 on generated and native H32/L2 checkpoints (2–4 cards). BF16 and interactive rendering remain unqualified. |
...
| Pokemon | Engine/core/data, semantic CPU model, evaluator and offline state-bank tools ported. Native observation binding, configuration, deterministic curriculum completion and per-checkpoint metadata hooks are implemented. FP32 native training passes with graphs off/on, snapshot resets, and fixed named expert banks; 2,048-step H8/L1 and configured H64/L2 learner integration tests pass against H16/L1 experts with legality auditing and immutable opponent files. CPU evaluator loads native checkpoints. Sequential round-robin orchestration/export is ported to state version 4 and tested through two real native rounds/evaluation; old state remains preserved, with compatible models reusable as seeds. Interactive visual qualification and BF16 remain unfinished; PFSP/QD and old custom optimizers are excluded. |
...
| Pokémon state banks / trained models | Regenerate banks with [offline collection tools](ocean/pokemon/README.md), or transfer selected weight/config pairs from `leagues/pokemon/free4/state.json`; see [preserved models](ocean/pokemon/README.md#preserved-user-models) | All 15 current `free4` weight/config pairs match their recorded hashes. Its ABI-3 128×2 master passes native continuation/evaluation with legality auditing. Older ABI-2 named-roster models are preserved but incompatible, not silently upgraded. Source data and pinned engine are in the port; no ROM required. |
