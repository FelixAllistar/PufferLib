...
| Retro | CPU emulator, compiled-ROM generator, practice/playback helpers and differential suites ported. Reference, compiled parity and strict ASan/UBSan tests pass. Emulator source has two local fixes for unaligned pixel access and uninitialized audio snapshot bytes. Full-screen CNN port passes exact legacy CPU traces at both resolutions and scale-4 FP32 GPU outputs/gradients/recurrent resets/graph-input tests. Native trainer/build integration, viewer, sweep panels and config remain unfinished; generic training explicitly blocked. BF16 unqualified. User ROM stays ignored/local. |
