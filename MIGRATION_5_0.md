...
| Bomberman | CPU simulator/curriculum, masks and play/watch viewer ported; CPU tests/sanitizers and FP32 CUDA-learner smoke pass. Legacy GPU simulator ported: 4,096 CPU/GPU match transitions pass, including non-default streams and graph replay; 2,048-step FP32 native training saves checkpoints with graphs off/on. GPU path retains legacy limitations: no masks, reverse curriculum or rendering; single-policy only. League workflows and exact old/new checkpoint inference parity remain. |
