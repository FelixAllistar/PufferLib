...
| Goofspiel | CPU/GPU simulator adapters, masks, renderer and CPU/CUDA standalone exact evaluators ported and tested. GPU parity covers 32,768 game transitions including policy rows, non-default streams and recreation. The independent GitHub checkout at `2e39262cc` builds the GPU trainer; 4,096-step four-bank smoke tests pass with graphs off/on and changed checkpoint weights. CUDA exact values match CPU within 5e-7 on generated and native H32/L2 checkpoints (2–4 cards). CPU-simulator native exact-response refresh/persistence is implemented using checkpoint/configure hooks plus an optional three-line checkpoint-load callback; sync/async continuation tests cover graphs off/on, reservoir saturation and byte-identical restore. GPU-simulator exact-response integration and exploitability-driven sweeps remain. |
