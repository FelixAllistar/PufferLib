#!/usr/bin/env python3
"""Export a standard native 5.0 MinGRU checkpoint to ONNX.

The checkpoint is the flat fp32 master-weight stream:
...
def read_env_schema(env_name: str) -> tuple[int, list[int]]:
    if env_name in {"kaggriculture", "retro", "pokemon", "shenaniguns3d", "nethack",
            "craftax", "nmmo3", "minimal", "asteroids", "osrs_colosseum",
            "osrs_inferno", "osrs_zulrah", "osrs_pvp", "webnav_dom"}:
        raise ValueError(f"{env_name} needs its custom-architecture exporter")
    ...
    command = [
        "cc", "-E", "-P", "-DPS_HEADLESS_BINDING", "-DAR_HEADLESS_BINDING",
        f"-I{ROOT / 'vendor'}", f"-I{ROOT.parent / 'box3d' / 'include'}",
    ...
    if all(size == 1 for size in action_sizes):
        raise ValueError("continuous log-standard-deviation export is not implemented")
...
def load_weights(checkpoint: Path, model: NativeMinGRU) -> dict[str, list[int]]:
    if model.hidden_size % 8:
        raise ValueError("export requires hidden_size divisible by 8 (native tensor alignment)")
    if checkpoint.stat().st_size % 4:
        raise ValueError("checkpoint has an incomplete fp32 value")
...
def main() -> int:
    ...
    metadata = {
        "format": "pufferlib-5.0-mingru-v1",
        "terminal_reset": "caller zeros state[:, terminal_rows, :] before next observation",
        "action_sampling": "caller applies environment legality masks and samples logits",
