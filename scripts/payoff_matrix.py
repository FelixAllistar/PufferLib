...
RESULT = re.compile(
    r"CUDA_EVAL env=(\S+) score=([0-9.eE+-]+) perf=([0-9.eE+-]+) games=(\d+)"
)
DRAW = re.compile(r"\bdraw_rate\s+([0-9.eE+-]+)")
...
def match(env: str, a: Path, b: Path, games: int,
        overrides: list[str], binary: Path) -> tuple[float, float, int]:
    command = [
        str(binary.resolve()), "match", "--headless",
        *("--" + value.removeprefix("--") for value in overrides),
        f"--base.load_model_path={a}",
        f"--base.load_enemy_model_path={b}",
        f"--base.eval_episodes={games}",
    ]
    ...
    draws = DRAW.findall(result.stdout)
    if not rows or not draws:
        raise RuntimeError(result.stdout)
    actual_env, score, _, completed = rows[-1]
    if actual_env != env:
        raise ValueError(f"binary runs {actual_env}, expected {env}")
    return float(score), float(draws[-1]), int(completed)
...
def main() -> int:
    ...
    for i in range(size):
        for j in range(i + 1, size):
            score_ij, draw_ij, games_ij = match(
                args.env, paths[i], paths[j], args.games, args.override, args.binary)
            score_ji, draw_ji, games_ji = match(
                args.env, paths[j], paths[i], args.games, args.override, args.binary)
            ...
            matches.append({
                ...
                "games_a_as_slot_0": games_ij, "games_b_as_slot_0": games_ji,
            })
    ...
    with output.with_suffix(".json").open("w") as file:
        ...
        json.dump({
            "env": args.env, "requested_games_per_orientation": args.games,
            "binary": str(args.binary.resolve()), "overrides": args.override,
            "draw_source": "dashboard (rounded to three decimals)",
