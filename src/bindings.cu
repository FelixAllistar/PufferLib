...
static void add_prio_debug(pybind11::dict& debug, const char* prefix,
        PrioBuffers& bufs, int prob_count, int idx_count, int weight_count) {
    auto key = [prefix](const char* suffix) {
        return pybind11::str(std::string(prefix) + suffix);
    };

    if (prob_count > 0 && bufs.prio_probs.data != nullptr) {
        std::vector<float> probs(prob_count);
        cudaMemcpy(probs.data(), bufs.prio_probs.data,
            (size_t)prob_count * sizeof(float), cudaMemcpyDeviceToHost);
        ...
        double max_entropy = prob_count > 1 ? std::log((double)prob_count) : 1.0;

        debug[key("_prob_sum")] = sum;
        debug[key("_prob_max")] = max_prob;
        debug[key("_entropy_frac")] = entropy / max_entropy;
    }

    if (idx_count > 0 && bufs.idx.data != nullptr) {
        std::vector<int> idx(idx_count);
        cudaMemcpy(idx.data(), bufs.idx.data,
            (size_t)idx_count * sizeof(int), cudaMemcpyDeviceToHost);
        ...
        debug[key("_sample_n")] = idx_count;
        debug[key("_sample_mean_idx")] = (double)sum / (double)idx_count;
        debug[key("_sample_min_idx")] = min_idx;
        debug[key("_sample_max_idx")] = max_idx;
    }

    if (weight_count > 0 && bufs.mb_prio.data != nullptr) {
        std::vector<float> weights(weight_count);
        cudaMemcpy(weights.data(), bufs.mb_prio.data,
            (size_t)weight_count * sizeof(float), cudaMemcpyDeviceToHost);
        ...
        debug[key("_weight_mean")] = sum / (double)weight_count;
        debug[key("_weight_min")] = min_weight;
        debug[key("_weight_max")] = max_weight;
    }
}

static void add_train_adv_debug(pybind11::dict& debug, PrecisionTensor& advantages,
        PrioBuffers& bufs, int idx_count) {
    int rows = (int)advantages.shape[0];
    int horizon = (int)advantages.shape[1];
    if (rows <= 0 || horizon <= 0 || advantages.data == nullptr) {
        return;
    }

    std::vector<precision_t> adv((size_t)rows * (size_t)horizon);
    cudaMemcpy(adv.data(), advantages.data,
        adv.size() * sizeof(precision_t), cudaMemcpyDeviceToHost);

    std::vector<float> row_sums(rows);
    double pop_sum = 0.0;
    float pop_max = 0.0f;
    for (int row = 0; row < rows; row++) {
        float sum = 0.0f;
        size_t offset = (size_t)row * (size_t)horizon;
        for (int t = 0; t < horizon; t++) {
            sum += fabsf(to_float(adv[offset + t]));
        }
        row_sums[row] = sum;
        pop_sum += sum;
        pop_max = std::max(pop_max, sum);
    }

    debug["train_adv_mean_abs_sum"] = pop_sum / (double)rows;
    debug["train_adv_max_abs_sum"] = pop_max;
    ...
    if (valid > 0) {
        double sample_mean = sample_sum / (double)valid;
        double pop_mean = pop_sum / (double)rows;
        debug["train_adv_sampled_mean_abs_sum"] = sample_mean;
        debug["train_adv_sampled_max_abs_sum"] = sample_max;
        debug["train_adv_sampled_mean_ratio"] =
            pop_mean > 0.0 ? sample_mean / pop_mean : 0.0;
    }
    debug["train_adv_sampled_valid_n"] = valid;
}

static void add_state_adv_debug(pybind11::dict& debug, StateBuffer& buf) {
    if (buf.size <= 0 || buf.advantages.data == nullptr) {
        return;
    }

    std::vector<precision_t> adv(buf.size);
    cudaMemcpy(adv.data(), buf.advantages.data,
        (size_t)buf.size * sizeof(precision_t), cudaMemcpyDeviceToHost);

    double pop_sum = 0.0;
    float pop_max = 0.0f;
    for (int i = 0; i < buf.size; i++) {
        float val = fabsf(to_float(adv[i]));
        pop_sum += val;
        pop_max = std::max(pop_max, val);
    }

    debug["state_adv_mean_abs"] = pop_sum / (double)buf.size;
    debug["state_adv_max_abs"] = pop_max;

    int idx_count = buf.num_cl_envs;
    if (idx_count <= 0 || buf.prio_bufs.idx.data == nullptr) {
        return;
    }

    std::vector<int> idx(idx_count);
    cudaMemcpy(idx.data(), buf.prio_bufs.idx.data,
        (size_t)idx_count * sizeof(int), cudaMemcpyDeviceToHost);
    ...
    for (int i = 0; i < idx_count; i++) {
        int state_idx = idx[i];
        if (state_idx < 0 || state_idx >= buf.size) {
            continue;
        }
        float val = fabsf(to_float(adv[state_idx]));
        sample_sum += val;
        sample_max = std::max(sample_max, val);
        valid++;
    }
    ...
    if (valid > 0) {
        double sample_mean = sample_sum / (double)valid;
        double pop_mean = pop_sum / (double)buf.size;
        debug["state_adv_sampled_mean_abs"] = sample_mean;
        debug["state_adv_sampled_max_abs"] = sample_max;
        debug["state_adv_sampled_mean_ratio"] =
            pop_mean > 0.0 ? sample_mean / pop_mean : 0.0;
    }
    debug["state_adv_sampled_valid_n"] = valid;
}
...
pybind11::dict puf_log(pybind11::object pufferl_obj) {
    ...
    // Debug surface for PER/CL wiring. These are intentionally cheap enough to
    // leave on while sweeping and are not printed in the terminal dashboard.
    pybind11::dict debug_dict;
    HypersT& h = pufferl.hypers;
    int batch_size = h.total_agents * h.horizon;
    int total_epochs = batch_size > 0 ? h.total_timesteps / batch_size : 0;
    float progress = total_epochs > 0 ? (float)pufferl.epoch / (float)total_epochs : 1.0f;
    progress = std::max(0.0f, std::min(1.0f, progress));
    float current_cl_frac = h.anneal_cl ? h.cl_frac * (1.0f - progress) : h.cl_frac;
    int configured_cl_envs = clamp_int((int)(current_cl_frac * (float)pufferl.vec->size),
        0, pufferl.vec->size);
    float anneal_beta = h.prio_beta0;
    if (h.anneal_prio_beta && total_epochs > 0) {
        anneal_beta += (1.0f - h.prio_beta0) * h.prio_alpha
            * (float)pufferl.epoch / (float)total_epochs;
    }

    debug_dict["curriculum_enabled"] = pufferl.curriculum_enabled;
    debug_dict["current_cl_frac"] = current_cl_frac;
    debug_dict["configured_cl_envs"] = configured_cl_envs;
    debug_dict["log_env_limit"] = pufferl.vec->log_env_limit;
    debug_dict["prio_alpha"] = h.prio_alpha;
    debug_dict["anneal_prio_beta"] = h.anneal_prio_beta;
    ...
    add_prio_debug(debug_dict, "train_per", pufferl.prio_bufs,
        h.total_agents, minibatch_segments, minibatch_segments);
    add_train_adv_debug(debug_dict, pufferl.advantages_puf,
        pufferl.prio_bufs, minibatch_segments);

    if (pufferl.curriculum_enabled) {
        StateBuffer& buf = pufferl.state_buf;
        debug_dict["state_buffer_fill"] = buf.size;
        ...
        add_prio_debug(debug_dict, "state_per", buf.prio_bufs,
            0, buf.num_cl_envs, 0);
        add_state_adv_debug(debug_dict, buf);
    } else {
        debug_dict["state_buffer_fill"] = 0;
        debug_dict["state_buffer_capacity"] = 0;
        debug_dict["state_num_fresh_envs"] = pufferl.vec->size;
        debug_dict["state_num_cl_envs"] = 0;
        debug_dict["state_warmup_active"] = false;
    }
    result["debug"] = debug_dict;
