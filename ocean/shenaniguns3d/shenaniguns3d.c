...
// trainable implementation lives in ocean/shenaniguns3d/encoder.cu; these constants are
...
static int s3d_eval_deterministic_from_config(void) {
    Ini ini = { 0 };
    puf_ini_load_env(&ini, S3D_ENV_NAME, 0, NULL);
    DictItem* option = dict_find(puf_ini_section(&ini, "base", 0), "eval_deterministic");
    int deterministic = option && option->value != 0;
    puf_ini_free(&ini);
...
static int headless_eval(S3DPolicy* net, int episodes, int deterministic, int trace) {
    ...
    for (int episode = 0; episode < episodes; episode++) {
        ...
        for (int tick = 0; tick <= env.max_ticks; tick++) {
            ...
            if (deterministic) {
                multidiscrete(net->base->multidiscrete,
                                     net->base->decoder->output, actions, 1, NULL);
            } else {
