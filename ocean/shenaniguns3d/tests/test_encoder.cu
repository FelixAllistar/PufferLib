...
static void init_test(TestState* test) {
    ...
    alloc_create(&test->acts);
    alloc_create(&test->grads);

    test->input.shape[0] = test->batch;
    test->input.shape[1] = OBS_SIZE;
    test->input.data = (precision_t*)NULL;
...
static double forward_loss(TestState* test) {
    Prec output = test->encoder.forward(test->weights, test->train_acts,
        test->input, 0);
...
static void cpu_arch_forward(const float* input, const float* scalar1_w,
        const float* scalar1_b, const float* scalar2_w, const float* scalar2_b,
        const float* depth_w, const float* depth_b, const float* occupancy_w,
        const float* occupancy_b, const float* proj_w, const float* proj_b,
...
static void fill_device_tensor(Prec tensor, int seed, float scale,
        float bias) {
...
static void initialize_full_policy(FullPolicyTest* test) {
    ...
    test->policy = build_arch(OBS_SIZE, test->hidden,
        test->layers, 15, false, 4);
    test->weights = weights_create(&test->policy, &test->params);
    test->rollout_activations = arch_reg_rollout(&test->policy, test->weights,
...
static void check_full_forward() {
    ...
    for (int step = 0; step < 5; step++) {
        ...
        Prec native = arch_forward(&test.policy, test.weights,
        ...
        check_cuda(cudaMemcpy(native_state.data(), test.state.data,
            ...
            "copy full state");
        cpu_arch_forward(input.data(), scalar1_w.data(), scalar1_b.data(),
...
// Exercise the network chain without PPO importance/value caches: this test
// supplies its own output cotangents, not an optimizer loss.
static Prec train_network(FullPolicyTest* test, Activations& a,
        Prec input, Prec state, Prec terminals) {
    int batch = input.shape[0], horizon = input.shape[1];
    Arch* p = &test->policy;
    Weights w = test->weights;
    Prec h = p->encoder.forward(w.encoder, a.encoder, *puf_squeeze(&input, 0), 0);
    h = p->network.forward_train(w.network, *puf_unsqueeze(&h, 0, batch, horizon),
        state, terminals, a.network, 0, 0);
    return p->decoder.forward(w.decoder, a.decoder, *puf_squeeze(&h, 0), 0);
}

static double train_policy_loss(FullPolicyTest* test, Activations& activations,
        Prec input, Prec state, Prec terminals,
        Float grad_logits, Float grad_value, int batch, int horizon,
        int decoder_output) {
    Prec output = train_network(test, activations, input, state, terminals);
...
static void check_policy_gradients() {
    ...
    Activations train_activations = arch_reg_train(&test.policy,
        test.weights, &train_acts_alloc, &train_grads_alloc, batch * horizon);
    alloc_create(&train_acts_alloc);
    alloc_create(&train_grads_alloc);
    ...
    train_network(&test, train_activations, input, state, terminals);
    Float empty = {};
    arch_backward(&test.policy, test.weights, train_activations,
