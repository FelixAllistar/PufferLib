...
static PufferNet* ar_load_policy(const char* path,Weights** out) {
    int hidden=puf_ini_get(&g_controls_ini,"policy","hidden_size");
    int layers=puf_ini_get(&g_controls_ini,"policy","num_layers");
...
static void ar_policy_step(PufferNet* net,float* obs,float* actions,int deterministic) {
    if(!deterministic) {
        float terminals[1]={0};
        forward_puffernet(net,obs,actions,NULL,terminals);
        return;
    }
