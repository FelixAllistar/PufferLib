...
// -----------------------------------------------------------------------------
// This file owns the internal SoA state and kernels used by the native 5.0
// vector implementation in arpg.cu.
//
...
static inline void ar_cuda_alloc(ARCudaSim* sim, int num_envs, ARConfig cfg) {
    ...
    AR_BLOB_FIELD(int, enemy_dense_pos, NE); AR_BLOB_FIELD(int, enemy_slow_timer, NE);
    ...
    if (state_offset != state_bytes) {
        fprintf(stderr, "ARPG state layout mismatch: %zu allocated, %zu used\n", state_bytes, state_offset);
        abort();
    }
}
