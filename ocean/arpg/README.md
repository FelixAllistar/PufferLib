...
Do not run the clone/checkout commands over an existing dirty dependency.
Save games under `saves/arpg/` are user assets, not bundled checkpoints.
...
The legacy GPU trainer used bounded five-minute segments, not the CPU campaign's
...
The 5.0 CPU simulator with GPU inference/PPO builds with:

```sh
CUDA_HOME=/usr/local/cuda ./build.sh arpg puffer_arpg --float
./puffer_arpg train
```
...
independent model tasks, camera projection/inversion,
