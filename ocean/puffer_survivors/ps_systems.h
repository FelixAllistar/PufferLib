...
#ifndef PS_SUCCESS_REWARD
#define PS_SUCCESS_REWARD 20.0f
#endif

#ifndef PS_LEVEL_UP_REWARD
#define PS_LEVEL_UP_REWARD 0.25f
#endif
...
static inline void ps_push_out_obstacles(PufferSurvivors* env, float* x, float* y, float radius, int penalize) {
    for (int i = 0; i < env->cfg.obstacle_count; i++) {
        ...
        *y += dy / d * push;
        if (penalize) {
            env->rewards[0] += env->cfg.obstacle_penalty;
...
static inline PSEnemyDef ps_enemy_stats(PufferSurvivors* env, int* kind_out, int elite, int boss) {
...
static inline int ps_spawn_enemy(PufferSurvivors* env) {
    ...
    int boss = env->tick > 0 && env->tick % 900 == 0 && env->last_boss_tick != env->tick;
    if (boss) env->last_boss_tick = env->tick;
    int kind = 0;
    PSEnemyDef stats = ps_enemy_stats(env, &kind, elite, boss);
...
static inline int ps_damage_enemy(PufferSurvivors* env, int eidx, float damage) {
    ...
    env->rewards[0] += env->cfg.reward_damage * damage;
    ...
    env->rewards[0] += env->cfg.reward_kill;
...
static inline void ps_update_enemies(PufferSurvivors* env) {
    ...
    for (int i = 0; i < env->cfg.enemy_cap; i++) {
        ...
        if (env->invuln_timer <= 0 && post_d2 < hit * hit) {
            ...
            env->hp -= dmg;
            env->rewards[0] += env->cfg.reward_hurt * dmg;
...
static inline void ps_update_drops(PufferSurvivors* env) {
    ...
    while (k < env->drop_count) {
        ...
        if (dist2 < pickup2) {
            if (env->drops.type[i] == 1) {
                ...
                env->rewards[0] += env->cfg.reward_xp * env->drops.value[i];
    ...
    while (env->xp >= ps_xp_threshold(env)) {
        ...
        env->level++;
        env->rewards[0] += PS_LEVEL_UP_REWARD;
        env->queued_upgrades++;
...
static inline void c_step(PufferSurvivors* env) {
    env->rewards[0] = env->cfg.reward_survival;
    env->terminals[0] = 0.0f;
    env->tick++;
    if (env->invuln_timer > 0) env->invuln_timer--;

    int upgrade_action = (int)env->actions[1];
