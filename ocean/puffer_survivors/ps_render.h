...
static inline void ps_draw_enemy(PufferSurvivors* env, int i, float scale, int w, int h) {
    ...
    if (ari_k) {
        float size = env->enemies.radius[i] * scale * 6.0f;
        DrawRectangle((int)(p.x - size * 0.5f), (int)(p.y - size * 0.5f), (int)size, (int)size, (Color){0, 0, 0, 255});
        DrawRectangleLines((int)(p.x - size * 0.5f), (int)(p.y - size * 0.5f), (int)size, (int)size, (Color){95, 95, 110, 255});
        int font_size = (int)ps_clampf(size * 0.22f, 18.0f, 36.0f);
    ...
    if (elite || boss || env->enemies.hp[i] < env->enemies.max_hp[i]) {
        float bw = env->enemies.radius[i] * scale * (ari_k ? 5.5f : (boss ? 3.2f : 2.3f));
        float bh = boss ? 6.0f : 4.0f;
        float pct = env->enemies.max_hp[i] > 0.0f ? env->enemies.hp[i] / env->enemies.max_hp[i] : 0.0f;
        float bar_y = p.y - env->enemies.radius[i] * scale * (ari_k ? 3.25f : 2.0f);
