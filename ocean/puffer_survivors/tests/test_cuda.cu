...
int main() {
    ...
    Env* envs = puf_vec_create(1, cfg, observations, actions, rewards, terminals);
    ...
    // The upstream evaluator may destroy and recreate the vector in one process.
    envs = puf_vec_create(1, cfg, observations, actions, rewards, terminals);
    puf_reset(envs);
    puf_step(envs);
    CUDA_CHECK(cudaDeviceSynchronize());
    CUDA_CHECK(cudaMemcpy(&terminal, terminals, sizeof(float), cudaMemcpyDeviceToHost));
