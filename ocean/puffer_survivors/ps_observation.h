...
static inline void ps_compute_observations(PufferSurvivors* env) {
    ...
    obs[idx++] = 1.0f - ps_clampf(env->weapon_cd[PS_WEAPON_BUBBLE] / ps_weapon_cooldown_total(env, PS_WEAPON_BUBBLE), 0.0f, 1.0f);
    ...
    if (nearest_xp_d2 < 1e29f) {
        ...
        obs[nearest_xp_proximity_idx] = 1.0f - ps_clampf(nearest_xp_dist * inv_observe_radius, 0.0f, 1.0f);
    }
    if (nearest_health_d2 < 1e29f) {
        float nearest_health_dist = sqrtf(nearest_health_d2);
        obs[nearest_health_proximity_idx] = 1.0f - ps_clampf(nearest_health_dist * inv_observe_radius, 0.0f, 1.0f);
    }
    ...
    for (int k = 0; k < env->projectile_count; k++) {
        ...
        if (ps_dist2(env->projectiles.x[i], env->projectiles.y[i], env->px, env->py) <= observe_radius2) {
            visible_projectiles++;
        }
    }
    ...
    obs[visible_projectiles_idx] = ps_clampf((float)visible_projectiles * inv_projectile_cap, 0.0f, 1.0f);
