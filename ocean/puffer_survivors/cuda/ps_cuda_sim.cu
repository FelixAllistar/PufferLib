...
#define PS_MIDX(sim, action, env) ((action) * (sim).num_envs + (env))
...
struct PSCudaSim {
    ...
    float *episode_upgrade_counts;
    float *episode_move_counts;
    ...
    float *log_upgrade_counts, *log_win_upgrade_counts;
    float *log_move_counts, *log_win_move_counts;
};
...
static inline PSConfig ps_sanitize_config(PSConfig cfg) {
    ...
    if (cfg.observation_version < 6 || cfg.observation_version > 9)
        cfg.observation_version = 9;
    if (cfg.free_upgrade < -1 || cfg.free_upgrade >= PS_UPGRADE_COUNT) cfg.free_upgrade = -1;
    if (cfg.free_upgrade_count < 0) cfg.free_upgrade_count = 0;
    return cfg;
}
...
static inline void ps_cuda_alloc(PSCudaSim* sim, int num_envs, PSConfig cfg) {
    ...
    PS_ALLOC_FIELD(sim, episode_upgrade_counts, N * PS_UPGRADE_COUNT);
    PS_ALLOC_FIELD(sim, episode_move_counts, N * 9);
    ...
    PS_ALLOC_FIELD(sim, log_upgrade_counts, N * PS_UPGRADE_COUNT); PS_ALLOC_FIELD(sim, log_win_upgrade_counts, N * PS_UPGRADE_COUNT);
    PS_ALLOC_FIELD(sim, log_move_counts, N * 9); PS_ALLOC_FIELD(sim, log_win_move_counts, N * 9);
    ...
    PS_ZERO_FIELD(sim, episode_upgrade_counts, N * PS_UPGRADE_COUNT);
    PS_ZERO_FIELD(sim, episode_move_counts, N * 9);
    ...
    PS_ZERO_FIELD(sim, log_upgrade_counts, N * PS_UPGRADE_COUNT); PS_ZERO_FIELD(sim, log_win_upgrade_counts, N * PS_UPGRADE_COUNT);
    PS_ZERO_FIELD(sim, log_move_counts, N * 9); PS_ZERO_FIELD(sim, log_win_move_counts, N * 9);
}
...
PS_D void ps_compute_observations(const PSCudaSim& sim, int env) {
    ...
    if (sim.cfg.observation_version == 8 && nearest_obstacle_d2 < 1e29f) {
        ...
        ps_obs_set(sim, env, alt_d_idx, ps_clampf(nearest_obstacle_radius / 1.1f, 0.0f, 1.0f));
    }
...
PS_D void ps_apply_upgrade(const PSCudaSim& sim, int env, int choice) {
    ...
    sim.episode_upgrade_counts[PS_UIDX(sim, upgrade, env)] += 1.0f;
...
PS_D void ps_add_log(const PSCudaSim& sim, int env, int survived) {
    ...
    for (int i = 0; i < PS_UPGRADE_COUNT; i++) {
        float count = sim.episode_upgrade_counts[PS_UIDX(sim, i, env)];
        sim.log_upgrade_counts[PS_UIDX(sim, i, env)] += count;
        if (survived) sim.log_win_upgrade_counts[PS_UIDX(sim, i, env)] += count;
    }
    for (int i = 0; i < 9; i++) {
        float count = sim.episode_move_counts[PS_MIDX(sim, i, env)];
        sim.log_move_counts[PS_MIDX(sim, i, env)] += count;
        if (survived) sim.log_win_move_counts[PS_MIDX(sim, i, env)] += count;
    }
    if (survived) {
...
PS_D PSEnemyDef ps_enemy_stats(const PSCudaSim& sim, int env, int* kind_out, int elite, int boss, int ari_k) {
    int wave = ps_wave_index(sim, env);
    float progress = ps_episode_progress(sim, env);
    int kind = 0;
    if (wave >= 2) {
    ...
    if (ari_k) {
        stats.hp *= 2.25f;
        stats.speed_mult = sim.cfg.enemy_speed * 0.48f;
        stats.radius = 1.55f;
        stats.damage = 6.0f * sim.cfg.enemy_damage_scale;
    }
...
PS_D int ps_spawn_enemy(const PSCudaSim& sim, int env) {
    ...
    int boss = ari_k || (sim.tick[env] > 0 && sim.tick[env] % 900 == 0
        && sim.last_boss_tick[env] != sim.tick[env]);
    if (boss) sim.last_boss_tick[env] = sim.tick[env];
    int kind = 0;
    PSEnemyDef stats = ps_enemy_stats(sim, env, &kind, elite, boss, ari_k);
...
PS_D void ps_wave_spawns(const PSCudaSim& sim, int env) {
    ...
    int wave = ps_wave_index(sim, env);
    if (local == 1 && wave >= 29 && (wave - 29) % 5 == 0) ps_spawn_enemy(sim, env);
...
PS_D void ps_reset_env(const PSCudaSim& sim, int env, int clear_outputs) {
    ...
    for (int i = 0; i < PS_UPGRADE_COUNT; i++)
        sim.episode_upgrade_counts[PS_UIDX(sim, i, env)] = 0.0f;
    for (int i = 0; i < 9; i++)
        sim.episode_move_counts[PS_MIDX(sim, i, env)] = 0.0f;
    if (sim.cfg.free_upgrade >= 0 && sim.cfg.free_upgrade < PS_UPGRADE_COUNT) {
...
PS_D void ps_step_env(const PSCudaSim& sim, int env) {
    ...
    sim.episode_move_counts[PS_MIDX(sim, action, env)] += 1.0f;
    float dx = 0.0f;
    float dy = 0.0f;
    switch (action) {
...
#undef PS_PACK_LOG
    for (int i = 0; i < PS_UPGRADE_COUNT; i++) {
        int idx = PS_UIDX(sim, i, env);
        record->upgrade_counts[i] += sim.log_upgrade_counts[idx];
        record->win_upgrade_counts[i] += sim.log_win_upgrade_counts[idx];
        sim.log_upgrade_counts[idx] = 0.0f;
        sim.log_win_upgrade_counts[idx] = 0.0f;
    }
    for (int i = 0; i < 9; i++) {
        int idx = PS_MIDX(sim, i, env);
        record->move_counts[i] += sim.log_move_counts[idx];
        record->win_move_counts[i] += sim.log_win_move_counts[idx];
        sim.log_move_counts[idx] = 0.0f;
        sim.log_win_move_counts[idx] = 0.0f;
    }
}
