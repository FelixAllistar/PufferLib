...
```bash
make -C ocean/shenaniguns demo
./ocean/shenaniguns/build/demo # WASD/arrows move+strafe, SPACE fire, R reset
./ocean/shenaniguns/build/demo --bench # headless SPS benchmark

./build.sh shenaniguns          # native train/eval binary (puffer trainer)
./puffer train                 # upstream 5.0; environment already compiled in
./build.sh shenaniguns --cpu    # generic checkpoint viewer/headless evaluator
```

The 5.0 port preserves the CPU simulator and playable demo without core changes.
It fixes a legacy adapter bug: auto-reset previously erased the final reward and
terminal flag. Timeout steps now also clear stale rewards before ending a game.
`make -C ocean/shenaniguns test sanitize` checks these transition contracts.
The new config makes the documented two-player defaults explicit; there was no
tracked 2D environment config in `5c`. Training uses upstream's network and PPO.
