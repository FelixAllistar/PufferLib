...
//   ./ocean/bomberman/build/viewer                          # human vs random
//   ./ocean/bomberman/build/viewer watch                    # latest checkpoint vs itself
//   ./ocean/bomberman/build/viewer watch path/to/model.bin  # that ckpt vs itself
//   ./ocean/bomberman/build/viewer watch A.bin B.bin        # visible A vs B
//   ./ocean/bomberman/build/viewer play  path/to/model.bin  # human vs that ckpt
//   ./ocean/bomberman/build/viewer bench
//
...
static const char* resolve_model_path(const char* arg, char* buf, size_t buf_sz) {
    ...
    if (!buf[0]) {
        ...
        fprintf(stderr, "Train first:  ./puffer train\n");
        return NULL;
    }
    return buf;
}
...
static int resolve_model_config_path(const char* model_path,
    ...
    if (marker) {
        int prefix_len = marker - model_path;
        marker += strlen("checkpoints/bomberman/");
        const char* run_end = strchr(marker, '/');
        if (run_end && run_end > marker) {
            snprintf(out, out_size, "%.*slogs/bomberman/%.*s.ini",
                prefix_len, model_path, (int)(run_end - marker), marker);
            if (regular_file_exists(out)) return 1;
        }
    }
...
static void load_model_config(const char* model_path, BMConfig* cfg,
    ...
    *hidden = puf_ini_get(&ini, "policy", "hidden_size");
    *layers = puf_ini_get(&ini, "policy", "num_layers");
    puf_ini_free(&ini);
}
...
// Use the upstream masked sampler for both joint and separate policies.
static void forward_masked_bomberman(PufferNet* net, Env* env,
        float* observations, float* actions, int first_agent) {
    unsigned char masks[BM_MAX_AGENTS * BM_NUM_ACTIONS];
    float terminals[BM_MAX_AGENTS] = {0};
    for (int i = 0; i < net->num_agents; i++) {
        for (int action = 0; action < BM_NUM_ACTIONS; action++) {
            masks[i * BM_NUM_ACTIONS + action] =
                bm_action_legal(&env->match, first_agent + i, action);
        }
    }
    forward_puffernet(net, observations + first_agent * OBS_SIZE,
        actions + first_agent, masks, terminals);
}
...
static void play_loop(int mode, const char* model_path,
    ...
    if (mode == 0) {
        ...
        printf("  For AI opponent:  ./ocean/bomberman/build/viewer play     (loads latest checkpoint)\n");
        printf("  For AI vs AI:     ./ocean/bomberman/build/viewer watch\n");
        printf("Controls: WASD/arrows, Space bomb, R reset, Esc quit\n");
    } else if (mode == 1) {
        printf("Human vs checkpoint (latest checkpoint unless a path is passed). Cyan=you, red=AI.\n");
        printf("Shift=AI controls you too. R reset, Esc quit.\n");
    } else {
        printf(enemy_net
            ? "Watch: policy A (cyan) vs policy B (red).\n"
            : "Watch: both agents use checkpoint (latest checkpoint unless a path is passed).\n");
    ...
    while (!WindowShouldClose()) {
        ...
        if (frame % frames_per_step == 0) {
            ...
            if (run_net) {
                if (enemy_net) {
                    forward_masked_bomberman(net, &env, observations, actions, 0);
                    forward_masked_bomberman(enemy_net, &env, observations, actions, 1);
                } else {
                    forward_masked_bomberman(net, &env, observations, actions, 0);
                }
            }
...
int main(int argc, char** argv) {
    ...
    if (strcmp(argv[1], "selftest") == 0) {
        const char* p = resolve_model_path(argc > 2
            ? argv[2] : "latest", path_buf, sizeof(path_buf));
        if (!p) return 1;
