...
function overrides(args) {
    ...
    for(const arg of args) {
        ...
        if(!(key in defaults()))fail('Override is not a current native setting: '+key);
...
export function bindingConfig(base,member) {
    const config={...base,...member.overrides,
        ...
        'base.env_name':'pokemon','base.async':'0','train.gpus':'1',
        'vec.num_buffers':'1','vec.action_mask_size':'168',
        'env.team_selection':'1','selfplay.enabled':'0','base.load_enemy_model_path':'None',
        'selfplay.initial_opponents':'None','selfplay.eval_pool_size':'0',
        'selfplay.eval_games':'0','selfplay.eval_bot_games':'0','base.eval_episodes':'0'};
...
function trainCommand(directory,state,member,config,runID,load,steps,league='None',selfplay=1) {
    ...
    const args=[state.binary,'train',...Object.entries(actual).map(([key,value])=>'--'+key+'='+value)];
...
export function nativeManifest(members,architecture,opponents) {
...
function writeManifest(file,members,architecture) {
    const opponents=file+'.opponents';
    atomic(opponents,members.map(m=>(m.current||m).path).join('\n')+'\n');
    atomic(file,nativeManifest(members,architecture,opponents));
}
...
export function planCreate(options={},overrideArgs=[]) {
    ...
    return {version:VERSION,rules_sha:RULES.sha256,created:timestamp(),phase:'creating',round:0,seed,architecture,
        binary:path.resolve(ROOT,options.binary||'puffer'),evaluator:path.resolve(ROOT,options.evaluator||'build/pokemon/viewer'),
    ...
    for(const member of state.members) {
        ...
        if(member.initializer) {
            ...
            const initConfig={...config,'vec.total_agents':'8','vec.num_threads':'1','train.horizon':'8','train.minibatch_size':'64',
                'train.learning_rate':'0','train.total_timesteps':'64',
...
async function trainRounds(directory,state,options) {
    ...
    for(let i=0;i<rounds;i++) {
        ...
        for(const member of state.members.filter(m=>m.trainable)) {
            ...
            if(pending.selfplay<1)writeManifest(cmd.bank,cmd.opponents,state.architecture);
            jsonWrite(path.join(directory,'plans',cmd.config['base.run_id']+'.json'),cmd);
...
This 5.0 tool uses state version 4; old version-3 league state is not resumed.
Seed a new directory from compatible old checkpoints instead.
See ocean/pokemon/README.md for native setup and preserved models.`);
}
...
export async function main(argv=process.argv.slice(2)) {
    ...
    if(command==='export') {
        ...
        if((exists(out) || exists(out+'.opponents')) && !options.force)fail('Output exists; use --force to replace '+out);
        writeManifest(out,selected,state.architecture);
