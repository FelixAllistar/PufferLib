...
// Exercise preserved environment callbacks directly, not native trainer setup.
// These removed legacy defaults are fixture data, not 5.0 training settings.
static void load_callback_fixture(Ini* ini) {
    puf_ini_load_env(ini,"pokemon",0,NULL);
    puf_ini_set(puf_ini_section(ini,"vec",0),"seat_balance","0");
    puf_ini_set(puf_ini_section(ini,"selfplay",0),"opponent_pool_weights","1");
    puf_ini_set(puf_ini_section(ini,"selfplay",0),"opponent_league","None");
}
...
static void test_experts(void) {
    Ini ini={0}; load_callback_fixture(&ini);
