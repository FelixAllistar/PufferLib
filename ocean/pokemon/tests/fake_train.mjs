...
if(args[0]==='eval') {
    ...
    if(args[0]!=='train' || args.slice(1).some(arg=>!arg.startsWith('--')))throw Error('Expected native 5.0 CLI');
    const config=Object.fromEntries(args.slice(1).map(arg=>{const eq=arg.indexOf('=');return [arg.slice(2,eq),arg.slice(eq+1)];}));
