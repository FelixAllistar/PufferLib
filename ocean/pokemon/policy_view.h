...
static void pk_load_policy(PKPolicy* policy, const char* arg, int emag,
    ...
    int h = puf_ini_get(&policy->ini, "policy", "hidden_size");
    int layers = puf_ini_get(&policy->ini, "policy", "num_layers");
    ...
    policy->net = pk_cpu_make(policy->weights,1,h,layers);
...
static int pk_policy_action(PKPolicy* policy, const uint8_t* obs, const uint8_t* mask, int deterministic) {
    ...
    PKCpuPolicy* net = policy->net;
    // Semantic unpacking is identical to CUDA, including category one-hots.
    for (int i = 0; i < PK_OBS; i++) policy->observations[i] = (float)obs[i];
    float* logits=pk_cpu_forward(net,policy->observations);
    float max = -INFINITY;
    int best = -1;
...
static void pk_free_policy(PKPolicy* policy) {
    if (policy->net) pk_cpu_free(policy->net);
    free(policy->weights);
