...
struct KagMLPActs {
    Prec input_aug, mid, mid_aug, out;
    Prec grad_out, grad_mid_aug, grad_mid, grad_input_aug, grad_input;
    Prec dw1, dw2;
};

__global__ void kag_zero_padding(precision_t* w, int rows, int features) {
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row < rows) {
        for (int c = features; c < KAG_AUG_WIDTH(features); c++) {
            w[row * KAG_AUG_WIDTH(features) + c] = from_float(0.0f);
        }
    }
}

void kag_mlp_params(KagMLPWeights* w, Allocator* alloc) {
...
void kag_mlp_init(KagMLPWeights* w, ulong* seed, cudaStream_t stream) {
    puf_kaiming_init(&w->w1, sqrtf(2.0f * KAG_AUG_WIDTH(w->in) / w->in), (*seed)++, stream);
    puf_kaiming_init(
        &w->w2, w->output_gain * sqrtf((float)KAG_AUG_WIDTH(w->mid) / w->mid), (*seed)++, stream);
...
void kag_mlp_acts(
    KagMLPWeights* w, KagMLPActs* a, Allocator* acts, Allocator* grads, int rows, bool input_grad) {
    *a = {};
...
__global__ void kag_pack_aug(
    precision_t* dst, const precision_t* src, int rows, int cols, bool relu) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= rows * KAG_AUG_WIDTH(cols)) {
        return;
    }
...
__global__ void kag_relu(precision_t* values, int n) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n && to_float(values[idx]) < 0.0f) {
        values[idx] = from_float(0.0f);
    }
}

Prec kag_mlp_forward(KagMLPWeights* w, KagMLPActs* a, cudaStream_t stream) {
    ...
    if (w->relu_out) {
        kag_relu<<<grid_size(rows * w->out), BLOCK_SIZE, 0, stream>>>(a->out.data, rows * w->out);
    }
    return a->out;
}

__global__ void kag_relu_backward(
    precision_t* dst, const precision_t* grad, const precision_t* output, int n, bool relu) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < n) {
        dst[idx] = from_float(relu && to_float(output[idx]) <= 0.0f ? 0.0f : to_float(grad[idx]));
    }
}

__global__ void kag_strip_aug(precision_t* dst, const precision_t* grad,
    ...
    dst[idx] =
        from_float(relu && to_float(output_aug[source]) <= 0.0f ? 0.0f : to_float(grad[source]));
}

Prec kag_mlp_backward(KagMLPWeights* w, KagMLPActs* a, Prec grad, cudaStream_t stream) {
    int rows = a->out.shape[0];
    ...
    if (a->grad_input.data) {
        ...
        kag_strip_aug<<<grid_size(rows * w->in), BLOCK_SIZE, 0, stream>>>(
            a->grad_input.data, a->grad_input_aug.data, NULL, rows, w->in, false);
    }
    return a->grad_input;
}
...
void kag_encoder_params(void* weights, Allocator* alloc) {
    KagEncoderWeights* w = (KagEncoderWeights*)weights;
    for (int i = 0; i < 4; i++) {
        kag_mlp_params(&w->entity[i], alloc);
    }
    kag_mlp_params(&w->fusion, alloc);
}
void kag_encoder_init(void* weights, ulong* seed, cudaStream_t stream) {
    KagEncoderWeights* w = (KagEncoderWeights*)weights;
    for (int i = 0; i < 4; i++) {
        kag_mlp_init(&w->entity[i], seed, stream);
    }
    kag_mlp_init(&w->fusion, seed, stream);
}
void kag_encoder_acts(
    void* weights, void* activations, Allocator* acts, Allocator* grads, int rows) {
    KagEncoderWeights* w = (KagEncoderWeights*)weights;
    KagEncoderActs* a = (KagEncoderActs*)activations;
    int entities[4] = {1, KAG_PRODUCT_COUNT, KAG_PLOT_COUNT, KAG_WORKER_COUNT};
    for (int i = 0; i < 4; i++) {
        kag_mlp_acts(&w->entity[i], &a->entity[i], acts, grads, rows * entities[i], false);
    }
    kag_mlp_acts(&w->fusion, &a->fusion, acts, grads, rows, true);
}
void kag_encoder_rollout(void* w, void* a, Allocator* alloc, int rows) {
    kag_encoder_acts(w, a, alloc, NULL, rows);
}
...
__global__ void kag_entity_input(precision_t* dst, const precision_t* obs, int rows, int features,
    ...
    if (f >= features) {
        dst[idx] = from_float(f == features ? 1.0f : 0.0f);
        return;
    }
...
__global__ void kag_fuse_entities(precision_t* dst, const precision_t* global,
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= rows * KAG_AUG_WIDTH(KAG_FUSION_WIDTH)) {
        return;
    }
    int r = idx / KAG_AUG_WIDTH(KAG_FUSION_WIDTH), f = idx % KAG_AUG_WIDTH(KAG_FUSION_WIDTH);
    if (f < 64) {
        dst[idx] = global[r * 64 + f];
    } else if (f < 64 + 288) {
        dst[idx] = product[r * 288 + f - 64];
    } else if (f < 64 + 288 + 256) {
        dst[idx] = plot[r * 256 + f - 64 - 288];
    } else if (f < KAG_FUSION_WIDTH) {
        dst[idx] = worker[r * 272 + f - 64 - 288 - 256];
    } else {
        dst[idx] = from_float(f == KAG_FUSION_WIDTH ? 1.0f : 0.0f);
    }
}

Prec kag_encoder_forward(void* weights, void* activations, Prec obs, cudaStream_t stream) {
    KagEncoderWeights* w = (KagEncoderWeights*)weights;
    KagEncoderActs* a = (KagEncoderActs*)activations;
    int rows = a->fusion.out.shape[0];
    int entities[4] = {1, 9, 8, 17};
    int offsets[4] = {0, KAG_PRODUCT_OFFSET, KAG_PLOT_OFFSET, KAG_WORKER_OFFSET};
    for (int i = 0; i < 4; i++) {
...
__global__ void kag_unfuse_gradient(
    precision_t* dst, const precision_t* fused, int rows, int width, int offset) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < rows * width) {
        dst[idx] = fused[(idx / width) * KAG_FUSION_WIDTH + offset + idx % width];
    }
}
void kag_encoder_backward(void* weights, void* activations, Prec grad, cudaStream_t stream) {
    KagEncoderWeights* w = (KagEncoderWeights*)weights;
    KagEncoderActs* a = (KagEncoderActs*)activations;
    Prec fused = kag_mlp_backward(&w->fusion, &a->fusion, grad, stream);
    int rows = a->fusion.out.shape[0], offset = 0;
    int widths[4] = {64, 288, 256, 272};
    for (int i = 0; i < 4; i++) {
...
struct KagDecoderWeights {
    // Upstream samples through a DecoderWeights pointer.
    DecoderWeights base;
...
void* kag_decoder_weights(void* self) {
    ...
    assert(!dec->continuous && dec->output_dim == KAG_ALL_LOGITS);
    assert(dec->hidden_dim >= 8 && dec->hidden_dim % 8 == 0);
...
void kag_decoder_params(void* weights, Allocator* alloc) {
    KagDecoderWeights* w = (KagDecoderWeights*)weights;
    for (int i = 0; i < 3; i++) {
        kag_mlp_params(&w->branch[i], alloc);
    }
}
void kag_decoder_init(void* weights, ulong* seed, cudaStream_t stream) {
    KagDecoderWeights* w = (KagDecoderWeights*)weights;
    for (int i = 0; i < 3; i++) {
        kag_mlp_init(&w->branch[i], seed, stream);
    }
}
void kag_decoder_acts(
    void* weights, void* activations, Allocator* acts, Allocator* grads, int rows) {
    KagDecoderWeights* w = (KagDecoderWeights*)weights;
...
__global__ void kag_merge_branches(precision_t* dst, const precision_t* tasks,
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= rows * (KAG_ALL_LOGITS + 1)) {
        return;
    }
    int r = idx / (KAG_ALL_LOGITS + 1), c = idx % (KAG_ALL_LOGITS + 1);
    dst[idx] = c < KAG_TASK_LOGITS  ? tasks[r * KAG_TASK_LOGITS + c]
               : c < KAG_ALL_LOGITS ? market[r * KAG_MARKET_LOGITS + c - KAG_TASK_LOGITS]
                                    : value[r];
}
Prec kag_decoder_forward(void* weights, void* activations, Prec input, cudaStream_t stream) {
    KagDecoderWeights* w = (KagDecoderWeights*)weights;
    KagDecoderActs* a = (KagDecoderActs*)activations;
    int rows = a->out.shape[0];
    for (int i = 0; i < 3; i++) {
...
__global__ void kag_branch_gradient(precision_t* dst, const float* logits, const float* value,
    int rows, int width, int offset, bool critic) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx < rows * width) {
        dst[idx] = from_float(
            critic ? value[idx] : logits[(idx / width) * KAG_ALL_LOGITS + offset + idx % width]);
    }
}
__global__ void kag_sum_branch_gradients(
