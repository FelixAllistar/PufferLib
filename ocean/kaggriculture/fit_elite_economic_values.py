...
def _write_ini(path: pathlib.Path, fit: dict[str, Any]) -> None:
    lines = [
        ...
        "# Every emitted training reward is nonnegative. These coefficients",
        "# are defaults, not constants: keep them in [env] and sweep freely.",
        "reward_progress_scale = 1.0",
        "reward_progress_terminal_money_scale = 0.25",
        ...
        "reward_progress_maintenance_scale = 1.0",
    ...
    for crop in CROPS:
        ...
        lines.append(
            ...
            f"{fit['seed_realization'][crop]:.6f}"
        )
    for animal in ANIMALS:
        ...
        lines.append(
            ...
            f"{fit['animal_realization'][animal]:.6f}"
        )
...
def _write_sweep(path: pathlib.Path) -> None:
    ...
    ranges = {
        "reward_progress_scale": (0.25, 2.0),
        "reward_progress_terminal_money_scale": (0.0, 2.0),
        "reward_progress_seed_scale": (0.0, 1.5),
        "reward_progress_crop_scale": (0.25, 2.0),
        "reward_progress_animal_scale": (0.25, 2.0),
        "reward_progress_product_scale": (0.25, 1.5),
