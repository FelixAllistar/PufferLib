...
The historical public Kaggle API served the index and raw archive without a
Kaggle CLI session; live endpoint access is not requalified by this port.
This tool uses that endpoint, skips
...
that want to audit an installed CLI, but this port exposes only direct HTTP.
"""
...
def _download_resumable(url: str, destination: pathlib.Path) -> tuple[int, bool]:
    ...
    try:
        ...
        if existing and error.code == 416 and error.headers.get('Content-Range') == f'bytes */{existing}':
            # The server confirms the partial file already has the full length.
            response = None
        else:
            raise
    if response is None:
        ...
        if append and not response.headers.get('Content-Range', '').startswith(f'bytes {existing}-'):
            response.close()
            raise ValueError('resumed download starts at the wrong offset')
        if not append:
            existing = 0
    ...
    if not zipfile.is_zipfile(temporary):
        raise ValueError(f'download is not a complete ZIP: {temporary}')
