...
profile_one() {
    ...
    printf '%s\t%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$label" "${kag_roles[$label]:-unassigned}" "${kag_weights[$label]:-0}" \
        "$completed" \
        "$(json_value "$json" env/perf)" "$(json_value "$json" env/draw_rate)" \
        "$(json_value "$json" env/score)" "$(json_value "$json" env/opponent_score)" \
        ...
        "$(json_value "$json" env/fertilizer_collect_action)" \
...
{
    printf 'policy\trole\tbase_weight\tgames_completed\twin_rate\tdraw_rate\tmean_money\topponent_money\twater_coverage\tneglect_deaths\tplanting_day_deaths\tplants_alive\tanimals_alive\tweeds\tland_purchases\tproductive_extra_tiles\tanimal_place_actions\tanimal_feed_actions\tanimal_care_actions\tanimal_harvest_actions\tfertilizer_collect_actions\torders_per_turn\tbuy_orders\tsell_orders\thire_orders\n'
