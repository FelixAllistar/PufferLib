...
struct Env {
    ...
    int curriculum_preunlocked_land;
...
static inline uint8_t kag_u8(int value) {
    return (uint8_t)(value < 0 ? 0 : value > 255 ? 255 : value);
}

static inline uint8_t kag_u8_scale(int value, int scale) {
    if (value <= 0) return 0;
    if (value >= scale) return 255;
    return (uint8_t)((value * 255) / scale);
}

static inline uint8_t kag_u8_signed(int value, int magnitude) {
    if (value <= -magnitude) return 0;
    if (value >= magnitude) return 255;
    return (uint8_t)(((value + magnitude) * 255) / (2 * magnitude));
}

static inline int kag_discrete_index(float value, int size) {
    int index = (int)value;
    if (index < 0) index = 0;
...
static inline float kag_player_potential(const Env* env, int player_id) {
    ...
    for (int word = 0; word < KG_TILE_WORDS; word++) {
        ...
        while (plants) {
            int tile = word * 64 + __builtin_ctzll(plants);
        ...
        while (animals) {
            int tile = word * 64 + __builtin_ctzll(animals);
    ...
    static const int land_value[4] = {0, 1000, 3000, 7000};
    int extra_land = __builtin_popcount((unsigned)player->unlocked_mask) - 1;
    assets += land_value[extra_land < 0 ? 0 : extra_land > 3 ? 3 : extra_land]
        * env->reward_land_value;
    return value + asset_scale * assets;
}

static inline uint8_t kag_tile_entity(const KGTile* tile) {
...
static inline void kag_encode_tile(const KGTile* tile, int day, obs_t* out) {
...
static inline void kag_encode_unit_routes(const KGState* game,
        const KGPlayer* farm, int ux, int uy, obs_t* out) {
    ...
    int best_distance[KAG_ROUTE_COUNT] = {999, 999, 999, 999, 999};
    int best_x[KAG_ROUTE_COUNT] = {ux, ux, ux, ux, ux};
    int best_y[KAG_ROUTE_COUNT] = {uy, uy, uy, uy, uy};
    int present = 0;
    for (int tile = 0; tile < KG_MAX_TILES; tile++) {
        int x = tile % KG_MAX_BOARD_SIZE;
        int y = tile / KG_MAX_BOARD_SIZE;
        const KGTile* target = &farm->tiles[tile];
        int route = -1;
        if ((target->kind == KG_TILE_PLANT && !target->watered_today)
                || (kg_is_animal_tile(target)
                    && (!target->fed_today || !target->cared_today))) {
            route = KAG_ROUTE_MAINTAIN;
        }
        int distance = abs(x - ux) + abs(y - uy);
        if (route >= 0 && distance < best_distance[route]) {
            best_distance[route] = distance;
            best_x[route] = x;
            best_y[route] = y;
            present |= 1 << route;
        }
        int harvestable = target->yield_units > 0
            && (kg_is_animal_tile(target)
                || (target->kind == KG_TILE_PLANT
                    && game->day - target->planted_day
                        >= KG_CROP_DEFS[target->crop].first_yield_day));
        if (harvestable && distance < best_distance[KAG_ROUTE_HARVEST]) {
            best_distance[KAG_ROUTE_HARVEST] = distance;
            best_x[KAG_ROUTE_HARVEST] = x;
            best_y[KAG_ROUTE_HARVEST] = y;
            present |= 1 << KAG_ROUTE_HARVEST;
        }
        if (target->kind == KG_TILE_EMPTY
                && distance < best_distance[KAG_ROUTE_EMPTY]) {
            best_distance[KAG_ROUTE_EMPTY] = distance;
            best_x[KAG_ROUTE_EMPTY] = x;
            best_y[KAG_ROUTE_EMPTY] = y;
            present |= 1 << KAG_ROUTE_EMPTY;
        }
        if (target->kind == KG_TILE_WEED
                && distance < best_distance[KAG_ROUTE_WEED]) {
            best_distance[KAG_ROUTE_WEED] = distance;
            best_x[KAG_ROUTE_WEED] = x;
            best_y[KAG_ROUTE_WEED] = y;
            present |= 1 << KAG_ROUTE_WEED;
        }
    }
    static const int shed_x[4] = {4, 5, 4, 5};
    static const int shed_y[4] = {4, 4, 5, 5};
    for (int access = 0; access < 4; access++) {
        int distance = abs(shed_x[access] - ux) + abs(shed_y[access] - uy);
        if (distance < best_distance[KAG_ROUTE_SHED]) {
            best_distance[KAG_ROUTE_SHED] = distance;
            best_x[KAG_ROUTE_SHED] = shed_x[access];
            best_y[KAG_ROUTE_SHED] = shed_y[access];
        }
    }
    present |= 1 << KAG_ROUTE_SHED;
    out[2] = (uint8_t)(present * 8);
    int k = 3;
    for (int route = 0; route < KAG_ROUTE_COUNT; route++) {
        out[k++] = kag_u8_signed(best_x[route] - ux, 9);
        out[k++] = kag_u8_signed(best_y[route] - uy, 9);
    }
}
...
static inline void kag_write_observation_with_summaries(Env* env, int player_id,
    ...
    for (int view = 0; view < KG_POLICY_UNIT_HEADS; view++) {
        ...
        if (count == 0) {
            ...
            int uy = sum_y / count;
            uint8_t routes[13];
            kag_encode_unit_routes(game, me, ux, uy, routes);
            ...
            static const uint8_t status_bits[5] = {128, 64, 32, 16, 8};
            for (int bit = 0; bit < 5; bit++) {
                out[k++] = (routes[1] & status_bits[bit]) ? 255 : 0;
            }
            for (int item = 0; item < KG_NUM_ITEMS; item++) {
                out[k++] = kag_u8_scale(inventory[item], 100);
            }
            int present = routes[2] / 8;
            for (int route = 0; route < 5; route++) {
                out[k++] = (present & (1 << route)) ? 255 : 0;
            }
            for (int route_coord = 0; route_coord < 10; route_coord++) {
                out[k++] = routes[3 + route_coord];
            }
        }
    }
    for (int view = 0; view < KG_POLICY_UNIT_HEADS; view++) {
...
static inline KGPolicyUnitSpec kag_unit_spec(int id) {
    static const int single_ops[9] = {
        KG_OP_WATER, KG_OP_HARVEST, KG_OP_FERTILIZE,
        KG_OP_BUILD_COOP, KG_OP_BUILD_PASTURE, KG_OP_DIG,
    ...
    if (id < KG_U_PLACE) {
        return (KGPolicyUnitSpec){single_ops[id - KG_U_SINGLE], -1, 1};
    }
    return (KGPolicyUnitSpec){KG_OP_PLACE, id - KG_U_PLACE, 1};
}

static inline KGPolicyMarketSpec kag_market_spec(int id) {
    static const int product_items[2] = {KG_ITEM_WHEAT, KG_ITEM_FERTILIZER};
    ...
    if (id < KG_M_ANIMAL) {
        return (KGPolicyMarketSpec){KG_MARKET_BUY_PRODUCT,
            product_items[id - KG_M_PRODUCT], 1};
    }
    if (id < KG_M_SELL) {
...
static inline int kag_unit_action_id(int op, int arg, int n) {
    (void)n;
...
static inline int kag_market_action_id(int op, int item, int n) {
    (void)n;
...
static inline int kag_market_quantity_id(int n) {
    static const int quantities[KG_POLICY_MARKET_QUANTITIES] = {
        1, 2, 3, 4, 5, 6, 8, 10};
    if (n < 0) return KG_POLICY_MARKET_QUANTITIES - 1;
    int best = 0;
    for (int id = 1; id < KG_POLICY_MARKET_QUANTITIES; id++) {
        if (quantities[id] > n) break;
        best = id;
    }
    return best;
}

static inline int kag_market_quantity_spec(int id) {
    static const int quantities[KG_POLICY_MARKET_QUANTITIES] = {
        1, 2, 3, 4, 5, 6, 8, 10};
...
static inline int kag_market_quantity(const KGState* game, const KGPlayer* player,
...
static inline int kag_unit_action_legal(const KGState* game, const KGPlayer* player,
    ...
    if (spec.op >= KG_OP_NORTH && spec.op <= KG_OP_WEST) {
        ...
        return kg_unit_can_move_to(player, x + dx[move], y + dy[move],
...
static inline int kag_market_action_legal(const KGState* game, const KGPlayer* player,
        KGPolicyMarketSpec spec) {
    int price = 0;
    if (spec.op == KG_MARKET_BUY_SEED) {
    ...
    if (spec.op == KG_MARKET_BUY_LAND) {
        ...
        return extra >= 0 && extra < 3 && player->money >= prices[extra];
    }
    return 0;
}

static inline void kag_write_unit_mask(const KGState* game, const KGPlayer* player,
    ...
    for (int move = 0; move < 4; move++) {
        if (kg_unit_can_move_to(player, x + dx[move], y + dy[move],
                game->config.board_size)) {
...
static inline void kag_write_market_mask(const KGState* game, const KGPlayer* player,
        unsigned char* mask) {
    for (int id = 0; id < KG_POLICY_MARKET_COMMANDS; id++) {
        KGPolicyMarketSpec spec = kag_market_spec(id);
...
static inline unsigned char* kag_market_slot_mask(unsigned char* mask, int slot) {
    return mask + KG_POLICY_MARKET_MASK_OFFSET
        + slot * KG_POLICY_MARKET_SLOT_MASK_SIZE;
}

static inline int kag_policy_market_slot_limit(const Env* env) {
    return env->policy_market_slots > 0
        ? env->policy_market_slots : KG_POLICY_MARKET_SLOTS;
}

static inline int kag_policy_hand_limit(const Env* env) {
    return env->policy_max_hands > 0
        ? env->policy_max_hands : KG_MAX_HANDS;
}

static inline void kag_write_market_slots(const Env* env,
...
static inline void kag_write_mask(Env* env, int player_id) {
    ...
    if (env->curriculum_stage == 1) {
        for (int slot = 0; slot < KG_POLICY_UNIT_HEADS; slot++) {
            unsigned char* unit_mask = mask + slot * KG_POLICY_UNIT_COMMANDS;
            memset(unit_mask, 0, KG_POLICY_UNIT_COMMANDS);
            unit_mask[KG_U_PASS] = 1;
            if (slot == 0) {
                const KGUnitState* unit = &player->units[0];
                const KGTile* tile = &player->tiles[
                    kg_tile_index(unit->x, unit->y)];
                if (tile->kind == KG_TILE_EMPTY
                        && player->seeds[KG_WHEAT] > 0) {
                    unit_mask[KG_U_PLANT + KG_WHEAT] = 1;
                } else if (tile->kind == KG_TILE_PLANT) {
                    if (!tile->watered_today) unit_mask[KG_U_SINGLE] = 1;
                    else if (tile->yield_units > 0) {
                        unit_mask[KG_U_SINGLE + 1] = 1;
                    }
                }
            }
        }
        for (int market_slot = 0; market_slot < KG_POLICY_MARKET_SLOTS;
                market_slot++) {
            unsigned char* slot_mask = kag_market_slot_mask(mask, market_slot);
            unsigned char* commands = slot_mask + KG_POLICY_MARKET_CONTINUE_ACTIONS;
            memset(commands, 0, KG_POLICY_MARKET_COMMANDS);
            int has_command = 0;
            for (int id = 0; id < KG_POLICY_MARKET_COMMANDS; id++) {
                KGPolicyMarketSpec spec = kag_market_spec(id);
                if (spec.op == KG_MARKET_SELL && spec.item == KG_ITEM_WHEAT
                        && spec.n == KG_POLICY_N_ALL
            ...
            slot_mask[0] = 1;
            slot_mask[1] = (unsigned char)has_command;
        }
    } else if (env->curriculum_stage == 2) {
        for (int slot = 0; slot < KG_POLICY_UNIT_HEADS; slot++) {
            unsigned char* unit_mask = mask + slot * KG_POLICY_UNIT_COMMANDS;
            unit_mask[KG_U_SINGLE + 3] = 0;
            unit_mask[KG_U_SINGLE + 4] = 0;
            unit_mask[KG_U_SINGLE + 6] = 0;
            unit_mask[KG_U_SINGLE + 7] = 0;
            unit_mask[KG_U_SINGLE + 8] = 0;
            for (int item = KG_ITEM_GOOSE; item <= KG_ITEM_SHEEP; item++) {
                unit_mask[KG_U_PLACE + item] = 0;
            }
        }
    } else if (env->curriculum_stage == 3) {
        for (int slot = 0; slot < KG_POLICY_UNIT_HEADS; slot++) {
            unsigned char* unit_mask = mask + slot * KG_POLICY_UNIT_COMMANDS;
            unit_mask[KG_U_SINGLE + 3] = 0;
            unit_mask[KG_U_SINGLE + 4] = 0;
    ...
    } else if (env->curriculum_stage == 4
            || env->curriculum_stage == 5) {
        /* Stage 4 learns a pre-unlocked 50-tile board. Stage 5 then learns
         * when to buy land. Livestock/build actions unlock in stage 6. */
        for (int slot = 0; slot < KG_POLICY_UNIT_HEADS; slot++) {
            unsigned char* unit_mask = mask + slot * KG_POLICY_UNIT_COMMANDS;
            unit_mask[KG_U_SINGLE + 3] = 0;
            unit_mask[KG_U_SINGLE + 4] = 0;
    ...
    /* Curriculum filters are teaching-task boundaries only. Stage 6, used for
     * full training, reaches this block with no command removed. */
    if (env->curriculum_stage >= 1 && env->curriculum_stage <= 5) {
        for (int market_slot = 0; market_slot < KG_POLICY_MARKET_SLOTS;
                market_slot++) {
            unsigned char* slot_mask = kag_market_slot_mask(mask, market_slot);
            unsigned char* commands = slot_mask + KG_POLICY_MARKET_CONTINUE_ACTIONS;
            ...
            for (int id = 0; id < KG_POLICY_MARKET_COMMANDS; id++) {
                ...
                if (env->curriculum_stage == 2) {
                    blocked = spec.op == KG_MARKET_BUY_SEED
                        || spec.op == KG_MARKET_BUY_PRODUCT
                        || spec.op == KG_MARKET_BUY_ANIMAL
                        || spec.op == KG_MARKET_BUY_LAND;
                } else if (env->curriculum_stage == 3) {
                    blocked = spec.op == KG_MARKET_BUY_PRODUCT
                        || spec.op == KG_MARKET_BUY_ANIMAL
                        || spec.op == KG_MARKET_BUY_LAND;
                } else if (env->curriculum_stage == 4
                        || env->curriculum_stage == 5) {
                    blocked = spec.op == KG_MARKET_BUY_PRODUCT
                        ...
                        || (env->curriculum_stage == 4
                            && spec.op == KG_MARKET_BUY_LAND)
                        || (env->curriculum_preunlocked_land
                            && spec.op == KG_MARKET_BUY_LAND);
                }
                if (blocked) commands[id] = 0;
                has_command |= commands[id] != 0;
            }
            slot_mask[0] = 1;
            slot_mask[1] = (unsigned char)has_command;
...
static inline void kag_apply_curriculum_reset(Env* env) {
    if (env->curriculum_stage == 1) {
        for (int player = 0; player < KG_NUM_PLAYERS; player++) {
            KGPlayer* farm = &env->game_storage.players[player];
            int tile = kg_tile_index(4, 4);
            farm->seeds[KG_WHEAT] = 4;
            kg_new_plant(farm, tile, KG_WHEAT, -2,
                env->game_storage.config.turns_per_day);
            farm->tiles[tile].yield_units = 1;
            farm->tiles[tile].consecutive_unwatered = 1;
        }
    } else if (env->curriculum_stage == 2) {
        for (int player = 0; player < KG_NUM_PLAYERS; player++) {
            for (int crop = 0; crop < KG_NUM_CROPS; crop++) {
                env->game_storage.players[player].seeds[crop] = 2;
            }
        }
    } else if (env->curriculum_stage == 3) {
        for (int player = 0; player < KG_NUM_PLAYERS; player++) {
            for (int crop = 0; crop < KG_NUM_CROPS; crop++) {
                env->game_storage.players[player].seeds[crop] = 1;
            }
        }
    }
    if (env->curriculum_preunlocked_land) {
        for (int player = 0; player < KG_NUM_PLAYERS; player++) {
            KGPlayer* farm = &env->game_storage.players[player];
            /* Mode 1 is the real paid 50-tile state. Mode 2 is a spatial
             * drill: only NE is productive, so a competent NW policy cannot
             * maximize reward by ignoring the newly exposed coordinates. */
            if (env->curriculum_preunlocked_land == 1) {
                farm->money -= 1000;
                farm->unlocked_mask |= 2;
            } else {
                farm->unlocked_mask = 2;
                for (int y = 0; y < 5; y++) {
                    for (int x = 0; x < 5; x++) {
                        farm->tiles[kg_tile_index(x, y)].kind = KG_TILE_LOCKED;
                    }
                }
                /* Daily farmer respawns occur at (4,4); retain this doorway
                 * so it can step east into the NE quadrant. */
                farm->tiles[kg_tile_index(4, 4)].kind = KG_TILE_EMPTY;
            }
            for (int y = 0; y < 5; y++) {
                for (int x = 5; x < 10; x++) {
                    KGTile* tile = &farm->tiles[kg_tile_index(x, y)];
                    if (tile->kind == KG_TILE_LOCKED) tile->kind = KG_TILE_EMPTY;
                }
            }
        }
    }
...
static inline KGUnitAction kag_decode_unit_action(const Agent* agent, int slot) {
...
static inline void kag_decode_action(KGAction* action, const Agent* agent,
...
static inline void kag_apply_policy_limits(const Env* env,
...
static inline void kag_starter_action(const KGState* game, int player_id,
        KGAction* action) {
    const KGPlayer* player = &game->players[player_id];
...
static inline void kag_bot_crop_rank(const KGState* game, int player_id,
    ...
    for (int active = 0; active < game->shop_count; active++) {
        ...
        while (products < KG_NUM_PRODUCTS
                && KG_SHOP_PRODUCTS[shop][products] >= 0) products++;
        int multiplier = products == 1 ? 2 : 1;
        for (int item = 0; item < products; item++) {
            int product = KG_SHOP_PRODUCTS[shop][item];
...
static inline int kag_bot_crop(int fixed_crop, int slot,
...
static inline int kag_bot_route(const KGPlayer* farm, const KGUnitState* unit,
        int tx, int ty) {
...
static inline int kag_bot_jobs(const KGState* game, int player_id,
    ...
    for (int tile_id = 0; tile_id < KG_MAX_TILES; tile_id++) {
        ...
        int crop_slot = slot++;
        static const uint8_t specialist_cap[KG_NUM_CROPS] = {
            25, 6, 16, 16, 10,
        };
        if (fixed_crop >= 0 && crop_slot >= specialist_cap[fixed_crop]) {
            continue;
        }
...
static inline void kag_bot_action(const KGState* game, int player_id,
    ...
    for (int worker = 0; worker < farm->unit_count; worker++) {
        ...
        for (int job = 0; job < job_count; job++) {
            ...
            int distance = abs((int)unit->x - jobs[job].x)
                + abs((int)unit->y - jobs[job].y);
    ...
    int land = __builtin_popcount((unsigned)farm->unlocked_mask);
...
static inline void kag_adaptive_market_add(KGAction* action, int limit,
        KGMarketOrder order) {
    if (action->market_count >= limit) return;
    action->market[action->market_count++] = order;
}

static inline void kag_adaptive_filter_market(KGAction* action, int keep_land,
        int keep_hire) {
    int kept = 0;
 ...
 */
static inline void kag_adaptive_action(const KGState* game, int player_id,
    ...
    if (profile == KAG_ADAPTIVE_SCENARIO
            ...
            && __builtin_popcount((unsigned)farm->unlocked_mask) < 3
            && farm->money >= 2500) {
        kag_adaptive_market_add(action, market_limit,
            (KGMarketOrder){KG_MARKET_BUY_LAND, -1, 1});
    }
...
static inline float kag_productive_action_credit(const KGState* game,
...
static inline void kag_log_actions(Env* env, const KGState* game,
        const KGAction* action) {
    const KGPlayer* farm = &game->players[0];
    env->log.market_orders += (float)action->market_count;
    for (int order = 0; order < action->market_count; order++) {
        int op = action->market[order].op;
...
void puf_init(Env* env, Dict* kwargs) {
    ...
    env->curriculum_preunlocked_land = (int)dict_get(
        kwargs, "curriculum_preunlocked_land");
    ...
    if ((opponent_hash >> 8)
        ...
        uint32_t script_cut = (uint32_t)(env->bot_script_fraction
            * 16777216.0f);
        uint32_t rules_cut = script_cut
            + (uint32_t)((1.0f - env->bot_script_fraction)
        ...
        if (type_value < script_cut) {
            env->bot_opponent = KAG_BOT_SCRIPT_BASE
                + (int)(type_hash % KG_SCRIPT_COUNT);
...
void puf_reset(Env* env) {
    ...
    kag_apply_curriculum_reset(env);
    for (int player = 0; player < KG_NUM_PLAYERS; player++) {
...
void puf_step(Env* env) {
    ...
    float curriculum_bonus[KG_NUM_PLAYERS] = {0.0f, 0.0f};
    ...
    for (int player = 0; player < KG_NUM_PLAYERS; player++) {
        ...
        if (bot == KAG_BOT_NONE && player == 1 && env->tag > 0) {
            bot = env->bot_opponent;
        }
        if (bot == KAG_BOT_STARTER) {
    ...
    if (env->curriculum_stage > 0 && env->curriculum_stage <= 3) {
        float scale = env->curriculum_stage == 1 ? 1.0f
            : env->curriculum_stage == 2 ? 0.25f : 0.10f;
        for (int player = 0; player < KG_NUM_PLAYERS; player++) {
            const KGPlayer* farm = &game->players[player];
            KGUnitAction* unit_action = &actions[player].farmer;
            int unit_count = farm->unit_count;
            for (int unit = 0; unit < unit_count; unit++) {
                if (unit > 0) unit_action = &actions[player].hands[unit - 1];
                KGPolicyUnitSpec spec = {
                    unit_action->op, unit_action->arg, unit_action->n,
                };
                if (!kag_unit_action_legal(game, farm, unit, spec)) continue;
                if (spec.op == KG_OP_WATER) curriculum_bonus[player] += 0.10f * scale;
                else if (spec.op == KG_OP_HARVEST) {
                    curriculum_bonus[player] += 0.10f * scale;
                } else if (spec.op == KG_OP_PLANT) {
                    curriculum_bonus[player] += 0.05f * scale;
                }
            }
    ...
    for (int player = 0; player < KG_NUM_PLAYERS; player++) {
        float reward = (env->potential[player] - before_potential[player])
            * env->reward_potential_scale + curriculum_bonus[player]
    ...
    if (done) {
        ...
        env->log.perf += win0;
        env->log.score += (float)p0;
        env->log.sweep_score += abs(p0 - game->config.starting_money) <= 2
            ? -3000.0f : (float)p0;
        env->log.opponent_score += (float)p1;
        ...
        env->log.land_purchases += (float)(
            __builtin_popcount((unsigned)game->players[0].unlocked_mask) - 1);
        env->log.water_coverage += game->plant_days[0] > 0
            ? (float)game->watered_plant_days[0] / game->plant_days[0] : 1.0f;
        env->log.neglect_deaths += (float)game->neglect_deaths[0];
        env->log.planting_day_deaths += (float)game->planting_day_deaths[0];
        for (int crop = 0; crop < KG_NUM_CROPS; crop++) {
            env->log.unused_seed_value += game->players[0].seeds[crop]
                * KG_CROP_DEFS[crop].seed_cost;
        }
        for (int tile = 0; tile < KG_MAX_TILES; tile++) {
            int x = tile % KG_MAX_BOARD_SIZE;
            int y = tile / KG_MAX_BOARD_SIZE;
            const KGTile* t = &game->players[0].tiles[tile];
