...
profile_one() {
    ...
    printf '%s\t%s\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
...
{
    printf 'policy\trole\tbase_weight\tgames_completed\twin_rate\tdraw_rate\tmean_money\topponent_money\twater_coverage\tneglect_deaths\tplanting_day_deaths\tplants_alive\tanimals_alive\tweeds\torders_per_turn\tbuy_orders\tsell_orders\thire_orders\n'
