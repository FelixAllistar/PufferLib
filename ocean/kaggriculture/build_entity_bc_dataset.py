...
def main():
    ...
    settings.read([root / "config/default.ini", root / "config/kaggriculture.ini", args.profile])
    effective = io.StringIO()
    settings.write(effective)
    profile = effective.getvalue()
    features, executor = 0, 2
