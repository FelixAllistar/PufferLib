...
/* Rule simulator from 5c; no policy, reward shaping, or trainer dependencies. */
#include <assert.h>
#include <stddef.h>
#include <stdint.h>
...
KG_HD const KGCropDef* kg_crop_defs(void) {
#ifdef __CUDA_ARCH__
...
KG_HD const KGAnimalDef* kg_animal_defs(void) {
#ifdef __CUDA_ARCH__
...
KG_HD const KGMarketDef* kg_market_defs(void) {
#ifdef __CUDA_ARCH__
...
KG_HD int kg_shop_product(int shop, int index) {
#ifdef __CUDA_ARCH__
...
KG_HD int kg_ctz64(uint64_t value) {
#ifdef __CUDA_ARCH__
...
KG_HD uint32_t kg_random_uint32(KGPythonRandom* r) {
    uint32_t y;
    int kk;
    if (r->index >= 624) {
...
KG_HD double kg_random_double(KGPythonRandom* r) {
    uint32_t a = kg_random_uint32(r) >> 5;
    uint32_t b = kg_random_uint32(r) >> 6;
    return ((double)a * 67108864.0 + (double)b) / 9007199254740992.0;
}

KG_HD int kg_random_below(KGPythonRandom* r, int n) {
    unsigned int k = 0;
    uint32_t value;
    int x = n;
    if (n <= 1) {
...
KG_HD int kg_quadrant(int x, int y, int board_size) {
...
KG_HD int kg_tile_index(int x, int y) {
    return y * KG_MAX_BOARD_SIZE + x;
}

KG_HD int kg_shed_access_count(int board_size, KGPosition out[4]) {
    int half = board_size / 2;
...
KG_HD int kg_is_shed_adjacent(const KGPosition* pos, int board_size) {
    KGPosition access[4];
    int i;
...
KG_HD KGPosition kg_default_spawn(int board_size) {
    KGPosition access[4];
    int i;
    kg_shed_access_count(board_size, access);
    for (i = 0; i < 4; i++) {
...
KG_HD void kg_clear_tile_data(KGTile* tile, int kind) {
...
KG_HD void kg_inventory_add(KGUnitState* unit, int item, int n) {
    int i;
...
KG_HD int kg_inventory_take(KGUnitState* unit, int item, int n) {
    int i;
...
KG_HD void kg_sync_public_positions(KGPlayer* player) {
    int i;
...
KG_HD void kg_set_unit_position(KGPlayer* player, int idx, int x, int y) {
...
KG_HD KGPosition kg_unit_position(const KGPlayer* player, int idx) {
    return (KGPosition){player->units[idx].x, player->units[idx].y};
}

KG_HD int kg_crop_product(int crop) {
    return crop >= 0 && crop < KG_NUM_CROPS ? crop : KG_ITEM_INVALID;
}

KG_HD int kg_animal_product(int animal) {
    return animal >= 0 && animal < KG_NUM_ANIMALS
        ? KG_ANIMAL_DEFS[animal].product : KG_ITEM_INVALID;
}

KG_HD void kg_new_plant(KGPlayer* player, int index, int crop, int day,
        int turns_per_day) {
...
KG_HD void kg_new_animal(KGPlayer* player, int index, int animal, int day) {
    KGTile* tile = &player->tiles[index];
...
KG_HD double kg_shape(int func, double x, double T) {
    double u;
    double over;
    if (x < 0.0) {
        x = 0.0;
...
KG_HD int kg_round_even(double value) {
    double lower;
    double fraction;
    if (value <= 0.0) {
        return 0;
...
KG_HD int kg_market_price(int product, int inventory) {
...
KG_HD void kg_refresh_prices(KGState* state) {
    int i;
...
KG_HD void kg_reset(KGState* state) {
    ...
    for (player_id = 0; player_id < KG_NUM_PLAYERS; player_id++) {
        ...
        state->rng_state[player_id] = (uint32_t)(state->config.seed
            + (uint64_t)player_id * 747796405U);
    }
}
...
KG_HD void kg_init(KGState* state, const KGConfig* config) {
    ...
    assert(state->config.episode_steps >= 1);
    assert(state->config.board_size >= 4 && state->config.board_size <= KG_MAX_BOARD_SIZE);
    assert(state->config.starting_money >= 0);
    assert(state->config.max_market_orders_per_turn >= 1
        && state->config.max_market_orders_per_turn <= KG_MAX_MARKET_ORDERS);
    assert(state->config.turns_per_day >= 1 && state->config.shed_capacity >= 1);
    assert(state->config.weed_spawn_chance >= 0.0);
    assert(state->config.town_shop_unlock_interval >= 1
        && state->config.town_shop_sell_interval >= 1
        && state->config.town_center_sell_interval >= 1);
    assert(state->config.farm_hand_cost_mult >= 0);
    kg_reset(state);
}
...
int kg_state_snapshot_valid(const KGState* state) {
    int p;
    if (state == NULL) {
        return 0;
    }
...
int kg_state_deserialize(KGState* state, const void* input, size_t input_size) {
    KGState candidate;
    if (state == NULL || input == NULL || input_size != sizeof(KGState)) {
        return 0;
    }
    memcpy(&candidate, input, sizeof(candidate));
    if (!kg_state_snapshot_valid(&candidate)) {
        return 0;
    }
...
KG_HD int kg_hire_cost(int n, int multiplier) {
    int fib = kg_fib(n);
    if (multiplier <= 0) {
        return 0;
    }
    if (fib > INT_MAX / multiplier) {
        return INT_MAX;
    }
    return fib * multiplier;
}

KG_HD KGPosition kg_spawn_hand(const KGPlayer* player, int board_size) {
    KGPosition access[4];
...
KG_HD void kg_do_hire(KGState* state, KGPlayer* player) {
    KGPosition spawn;
...
KG_HD void kg_do_buy_land(KGState* state, KGPlayer* player) {
...
KG_HD int kg_unit_can_move_to(const KGPlayer* player, int x, int y, int board_size) {
...
KG_HD void kg_drop_inventory(KGState* state, KGPlayer* player, KGUnitState* unit) {
    int i;
    int item;
    int count;
    int room;
...
KG_HD void kg_apply_unit_action(KGState* state, KGPlayer* player, int idx,
...
KG_HD void kg_daily_refresh_plants(KGState* state, KGPlayer* player, int current_day) {
    ...
    for (int word = 0; word < KG_TILE_WORDS; word++) {
        ...
        while (bits != 0) {
            ...
            int was_watered;
            bits &= bits - 1;
            if (x >= state->config.board_size || y >= state->config.board_size) {
                continue;
            }
            ...
            if (tile->yield_units > def->max_yield) {
                tile->yield_units = def->max_yield;
            }
            if (production_count == def->max_yield) {
...
KG_HD void kg_daily_refresh_animals(KGState* state, KGPlayer* player, int day) {
    int next_day = day + 1;
    for (int word = 0; word < KG_TILE_WORDS; word++) {
...
KG_HD void kg_decay_plants(KGState* state, KGPlayer* player, int step) {
    for (int word = 0; word < KG_TILE_WORDS; word++) {
        uint64_t bits = player->plant_bits[word];
        while (bits != 0) {
            int index = word * 64 + kg_ctz64(bits);
...
KG_HD void kg_spawn_weeds(KGState* state, KGPlayer* player, KGPythonRandom* rng) {
    int x;
    int y;
    for (y = 0; y < state->config.board_size; y++) {
        for (x = 0; x < state->config.board_size; x++) {
...
KG_HD void kg_end_of_day(KGState* state, int current_day) {
    KGPythonRandom rng;
    int player_id;
...
KG_HD int kg_quote_valid(int op, int item) {
    if (op == KG_MARKET_SELL) {
        return item >= 0 && item < KG_NUM_PRODUCTS;
    }
    if (op == KG_MARKET_BUY_PRODUCT) {
        return item == KG_ITEM_WHEAT || item == KG_ITEM_FERTILIZER;
    }
    if (op == KG_MARKET_BUY_SEED) {
        return item >= 0 && item < KG_NUM_CROPS;
    }
    if (op == KG_MARKET_BUY_ANIMAL) {
        return item >= KG_ITEM_GOOSE && item <= KG_ITEM_SHEEP;
    }
    return 0;
}

KG_HD int kg_commit_unit(KGState* state, int player_id, int op, int item, int price) {
    KGPlayer* player = &state->players[player_id];
    if (op == KG_MARKET_SELL) {
...
KG_HD void kg_process_market(KGState* state, const KGAction actions[KG_NUM_PLAYERS]) {
    ...
    for (i = 0; i < max_len; i++) {
        ...
        for (;;) {
            ...
            for (player_id = 0; player_id < KG_NUM_PLAYERS; player_id++) {
                if (!active[player_id] || orders[player_id].n <= 0) {
                    continue;
                }
        ...
        if (market_changed) {
            kg_refresh_prices(state);
        }
    }
}

KG_HD void kg_town_consume(KGState* state, int step) {
    int shop;
    int j;
    int market_changed = 0;
    if (step % state->config.town_shop_sell_interval == 0) {
        for (shop = 0; shop < state->shop_count; shop++) {
            ...
            int products = 0;
            while (products < KG_NUM_PRODUCTS && kg_shop_product(id, products) >= 0) {
                products++;
            }
...
KG_HD void kg_validate_plant_atomic(const KGAction* action, const KGPlayer* player,
 ...
 */
KG_HD void kg_rule_clear_action(KGAction* output, int hand_count) {
    int i;
...
KG_HD KGUnitAction* kg_rule_unit_action(KGAction* output, int unit) {
    if (unit == 0) {
        return &output->farmer;
    }
    if (unit > 0 && unit <= KG_MAX_HANDS) {
        return &output->hands[unit - 1];
    }
    return NULL;
}

KG_HD int kg_rule_distance(int x0, int y0, int x1, int y1) {
    int dx = x0 - x1;
    int dy = y0 - y1;
    if (dx < 0) {
        dx = -dx;
...
KG_HD void kg_rule_move_or_set(
    ...
    command->n = 0;
    if (x < tx) {
        command->op = KG_OP_EAST;
    } else if (x > tx) {
        command->op = KG_OP_WEST;
    } else if (y < ty) {
        command->op = KG_OP_SOUTH;
    } else if (y > ty) {
        command->op = KG_OP_NORTH;
    } else {
...
KG_HD void kg_rule_action_ex(
    ...
    for (int item = 0; item < KG_NUM_PRODUCTS && market_count < KG_MAX_MARKET_ORDERS; item++) {
        ...
        if (remaining <= liquidation_steps || price * 4 >= base * 5) {
            output->market[market_count++] = (KGMarketOrder){
                KG_MARKET_SELL, item,
                current->shed[item]};
            if (remaining > liquidation_steps) {
                break;
            }
        }
    }
    ...
    if (market_count == 0) {
        ...
        for (int index = 0; index < KG_MAX_TILES; index++) {
            if (current->tiles[index].kind == KG_TILE_EMPTY) {
                empty = 1;
            }
        }
        if (empty) {
    ...
    for (int unit = 0; unit < current->unit_count && unit <= KG_MAX_HANDS; unit++) {
        ...
        for (int index = 0; index < KG_MAX_TILES; index++) {
            ...
            if (used[index] || x >= state->config.board_size || y >= state->config.board_size) {
                continue;
            }
            if (tile->kind == KG_TILE_PLANT) {
                ...
                for (int crop = 0; crop < KG_NUM_CROPS; crop++) {
                    if (current->seeds[crop] > 0
                            && (arg < 0
                                || state->market.prices[crop] > state->market.prices[arg])) {
                        arg = crop;
                    }
                }
                if (arg >= 0) {
...
int kg_crop_from_name(const char* name) {
    int i;
    for (i = 0; i < KG_NUM_CROPS; i++) {
        if (name && strcmp(name, KG_CROP_NAMES[i]) == 0) {
            return i;
        }
    }
...
int kg_animal_from_name(const char* name) {
    int i;
    for (i = 0; i < KG_NUM_ANIMALS; i++) {
        if (name && strcmp(name, KG_ANIMAL_NAMES[i]) == 0) {
            return i;
        }
    }
...
int kg_item_from_name(const char* name) {
    int i;
    for (i = 0; i < KG_NUM_ITEMS; i++) {
        if (name && strcmp(name, KG_ITEM_NAMES[i]) == 0) {
            return i;
        }
    }
...
int kg_shop_from_name(const char* name) {
    int i;
    for (i = 0; i < KG_MAX_SHOPS; i++) {
        if (name && strcmp(name, KG_SHOP_NAMES[i]) == 0) {
            return i;
        }
    }
    return -1;
...
void kg_json_append(char** data, size_t* length, size_t* capacity,
        const char* format, ...) {
    va_list args;
    int needed;
    assert(*data);
    ...
    if ((size_t)needed + *length >= *capacity) {
        ...
        while ((size_t)needed + *length >= new_capacity) {
            new_capacity *= 2;
        }
...
void kg_json_tile(char** data, size_t* length, size_t* capacity, const KGTile* tile) {
    int kind = tile->kind;
...
void kg_json_player(char** data, size_t* length, size_t* capacity,
...
void kg_json_private(char** data, size_t* length, size_t* capacity,
...
const char* kg_snapshot_json(const KGState* state) {
    ...
    data[0] = 0;
    kg_json_append(&data, &length, &capacity,
        "{\"step\":%d,\"day\":%d,\"hour\":%d,\"done\":%s,\"farms\":[",
    ...
    for (i = 0; i < state->shop_count; i++) {
        ...
        kg_json_append(&data, &length, &capacity, "\"%s\"",
            KG_SHOP_NAMES[state->unlocked_shops[i]]);
    }
    kg_json_append(&data, &length, &capacity, "]}}");
    return data;
}
