...
typedef void RetroPolicy;
extern "C" {
RetroPolicy* retro_cpu_load(const char* path, int hidden, int layers);
void retro_cpu_act(RetroPolicy* policy, float* obs, float* action, bool deterministic);
void retro_cpu_reset(RetroPolicy* policy);
void retro_cpu_free(RetroPolicy* policy);
}
#include <omp.h>
#include <chrono>
#include <filesystem>
...
static void retro_timing_audit(Env* e,RetroPolicy* net,float* obs,float* action) {
    ...
    for(int frame=0;frame<6000;frame++) {
        ...
        retro_cpu_act(net,obs,action,true);
        puf_step(e);
...
int main(int argc,char** argv) {
    try {
        ...
        RetroPolicy* net=nullptr;
        if(watch) {
            std::string path=strcmp(checkpoint,"latest")?checkpoint:newest_checkpoint(&ini);
            size_t hidden=puf_ini_get(&ini,"policy","hidden_size");
            size_t layers=puf_ini_get(&ini,"policy","num_layers");
            ...
            net=retro_cpu_load(path.c_str(),hidden,layers);
            if(!net) throw std::runtime_error("checkpoint load failed");
        ...
        if(timing) {
            ...
            puf_close(&env); retro_cpu_free(net); puf_ini_free(&ini);
            return 0;
        }
        ...
        for(int i=0;display?!WindowShouldClose():i<600;i++) {
            if(watch) {
                retro_cpu_act(net,obs,&action,deterministic);
