...
```sh
OMP_WAIT_POLICY=PASSIVE ./puffer_retro_batch train retro
./retro watch latest --inspect
```

Config defaults to `load_model_path = None` for fresh CNN training. Change that variable to `latest` to load your saved CNN policy. Checkpoints live under `checkpoints/retro_cnn/retro`; earlier dense/crop weights are incompatible and never searched by default. Training and watch read the same checkpoint location from the config. Loading weights is a warm start, not an exact optimizer-state resume. Graceful Ctrl-C saves at the next safe boundary; SIGKILL cannot save. With the default save interval, the first periodic checkpoint appears at 1,310,720 decisions.
...
OMP_WAIT_POLICY=PASSIVE python3 tests/test_native_retro_runtime.py ./puffer_retro_batch
```

The native build produces `puffer_retro_batch`. There is no launch wrapper or automatic fresh/latest switching. The fast build produces `./retro`, including the inspector.
