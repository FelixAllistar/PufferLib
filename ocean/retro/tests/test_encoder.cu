...
#define PUFFER_RETRO
#define PUFFER_ENV_NAME "retro"
#include "../../../src/pufferl.cu"
#include "observation_reference.h"
extern "C" {
void* retro_reference_create(float*, int, int);
const float* retro_reference_encode(void*, const float*);
const float* retro_reference_logits(void*, const float*);
const float* retro_reference_image(void*, int);
void retro_reference_reset(void*);
void retro_reference_free(void*);
}
...
static void put(Prec t,const std::vector<float>& data) {
...
static void random_tensor(Prec t,unsigned seed,float scale,float offset=0) {
...
template<int Layer> static void convolution_parity(int B) {
    ...
    Allocator alloc={};
    Prec input={.shape={B,Layer?c.ih*c.iw*c.ic:RETRO_POLICY_INPUTS}};
    Prec columns={.shape={B*N,K}},old_out={.shape={B*N,c.oc}},new_out=old_out;
    Prec grad=old_out,old_dw={.shape={c.oc,K}},new_dw=old_dw;
    Prec old_db={.shape={c.oc}},new_db=old_db;
    Prec old_dx={.shape={B,c.ih*c.iw*c.ic}},new_dx=old_dx;
    RetroEncoderWeights ew={}; ew.w[Layer]=old_dw; ew.b[Layer]=old_db;
    Float partial={.shape={splits*c.oc*(K+1)}};
    ...
    alloc_register(&alloc,&partial); alloc_create(&alloc);
    ...
    auto compare=[&](Prec ref,Prec actual,const char* part,float atol,float rtol) {
...
static Prec reference_encoder_forward(void* w,void* activations,
...
static void borrowed_input_parity(Arch& policy,Weights& weights,int B) {
    ...
    Prec copy={.shape={B,RETRO_POLICY_INPUTS}};
    Prec seed_grad={.shape={B,policy.encoder.out_dim}},grad=seed_grad,owned_grad=seed_grad;
    ...
    for(auto* alloc:{&acts,&grads,&owned_acts,&owned_grads}) alloc_create(alloc);
    ...
    auto compare=[&](Prec input,const std::vector<float>& before) {
...
static void checkpoint_parity(const char* path,Arch policy,Weights weights,Prec flat) {
    ...
    Arch reference=policy; reference.encoder.forward=reference_encoder_forward;
    Allocator fast_alloc={},ref_alloc={};
    auto fast=arch_reg_rollout(&policy,weights,&fast_alloc,B);
    auto old=arch_reg_rollout(&reference,weights,&ref_alloc,B);
    ...
    Prec fast_state={.shape={L,B,H}},old_state=fast_state,input={.shape={B,RETRO_POLICY_INPUTS}};
    alloc_register(&fast_alloc,&fast_state); alloc_register(&fast_alloc,&input);
    alloc_register(&ref_alloc,&old_state); alloc_create(&fast_alloc); alloc_create(&ref_alloc);
    ...
    for(int step=0;step<128;step++) {
        ...
        auto actual=host(arch_forward(&policy,weights,fast,input,fast_state,0));
        auto expected=host(arch_forward(&reference,weights,old,input,old_state,0));
        for(int b=0;b<B;b++) {
...
int main(int argc,char** argv) {
    ...
    Arch policy=build_arch(RETRO_POLICY_INPUTS,H,L,64,false,4);
    Allocator params={},acts={},grads={},rollout_alloc={};
    auto weights=weights_create(&policy,&params); alloc_create(&params);
    ulong seed=73; weights_init(&policy,weights,&seed,0); ck(cudaDeviceSynchronize());
    ...
    Prec input={.shape={B,RETRO_POLICY_INPUTS}},grad={.shape={B,H}};
    alloc_register(&acts,&input); alloc_register(&acts,&grad);
    alloc_create(&acts); alloc_create(&grads);
    Prec flat={.data=(precision_t*)params.mem,.shape={params.total_bytes/(long)sizeof(precision_t)}};
    ...
    std::vector<void*> cpu(B);
    for(int b=0;b<B;b++) {
        cpu[b]=retro_reference_create(serialized.data(),H,L);
    }
    ...
    for(int b=0;b<B;b++) {
        auto* ref=retro_reference_encode(cpu[b],obs.data()+b*RETRO_POLICY_INPUTS);
        near(ref,gpu.data()+b*H,H,atol,rtol,"CNN/RAM/fusion encoder");
        for(int l=0;l<3;l++) {
            auto image=host(a.image[l]); int n=RETRO_CONVS[l].oh*RETRO_CONVS[l].ow*RETRO_CONVS[l].oc;
            near(retro_reference_image(cpu[b],l),image.data()+b*n,n,atol,rtol,"convolution feature map");
        }
    }
...
#endif
    auto rollout=arch_reg_rollout(&policy,weights,&rollout_alloc,B);
    Prec state={.shape={L,B,H}}; alloc_register(&rollout_alloc,&state);
    alloc_create(&rollout_alloc);
    for(int step=0;step<16;step++) {
        if(step==7) {
            ck(cudaMemset(state.data,0,L*B*H*sizeof(precision_t)));
            for(auto* p:cpu) retro_reference_reset(p);
        }
        for(int i=0;i<(int)obs.size();i++) obs[i]=0.5f+0.45f*sinf(i*0.139f+step*0.1f);
        put(input,obs); obs=host(input);
        auto out=host(arch_forward(&policy,weights,rollout,input,state,0));
        for(int b=0;b<B;b++) {
            const float* ref=retro_reference_logits(cpu[b],obs.data()+b*RETRO_POLICY_INPUTS);
    ...
    for(auto* p:cpu) retro_reference_free(p);
