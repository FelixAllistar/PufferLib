...
struct RetroEncoderWeights { Prec w[5],b[5]; int hidden; };
struct RetroEncoderActivations {
    Prec image[3], image_grad[3], ram_input, ram, ram_grad;
    Prec joined, joined_grad, out, saved_input, columns;
    Float weight_partials;
    Prec dw[5],db[5];
};
static Prec retro_matrix(Prec t,int rows,int cols) {
    return Prec{.data=t.data,.shape={rows,cols}};
}
...
static Prec retro_encoder_forward(void* w,void* activations,
...
static void retro_encoder_train(void* w,void* activations,Allocator* acts,Allocator* grads,int B) {
    ...
    for(int l=0;l<5;l++) {
        a->dw[l]=ew->w[l]; a->dw[l].data=NULL; a->db[l]=ew->b[l]; a->db[l].data=NULL;
...
static void* retro_encoder_create(void* self) {
    ...
    auto* ew=(RetroEncoderWeights*)calloc(1,sizeof(RetroEncoderWeights)); ew->hidden=e->out_dim;
    for(int l=0;l<3;l++) {
