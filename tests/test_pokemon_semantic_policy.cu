...
#define ENV_HEADER "ocean/pokemon/pokemon.h"
#define PUFFER_ENV_NAME "pokemon"
...
struct Test {
    static constexpr int B = 3, T = 4, H = 16, L = 2, O = PK_ACTIONS + 1;
    Policy policy;
    ...
    Test() {
        ...
        policy = build_policy("pokemon", PK_OBS, H, L, PK_ACTIONS, false, T);
        ...
        assert(params.num_regs == 12 + L && grads.num_regs == params.num_regs);
        ...
        for (int p = 0; p < params.num_regs; p++) {
            ...
            for (int i = 0; i < rows * cols; i++) {
                uint32_t bits=(uint32_t)i+0x9e3779b9u*(p+1);
                bits^=bits>>16;bits*=0x7feb352du;bits^=bits>>15;bits*=0x846ca68bu;bits^=bits>>16;
                float wave=(bits>>8)*0x1.0p-23f-1;
                host[i] = p < 12 ? 0.04f * wave : 0.03f + 0.005f * wave;
            }
            if (p < 12) {
                const int features[12] = {PK_MOVE_IN,64,PK_MON_IN,64,PK_FUSION,H,PK_SPECIES_IN,64,PK_MOVE_IN,64,H,H};
        ...
        input = tensor(nullptr, B, T, PK_OBS);
        roll_input = tensor(nullptr, B, PK_OBS);
        ...
        inputs.resize(B * T * PK_OBS);
        uint64_t action_rng=76102;
        for(int b=0;b<B;b++) {
            PKGame game={0};game.rng=42+b;game.draft=1;game.max_updates=512;pk_game_reset(&game);
            int skip=b==0?0:b==1?6:30;
            for(int j=0;j<skip;j++)pk_game_step(&game,pk_random_action(game.masks[0],&action_rng),pk_random_action(game.masks[1],&action_rng));
            for(int t=0;t<T;t++) {
                for(int f=0;f<PK_OBS;f++)inputs[(b*T+t)*PK_OBS+f]=game.obs[0][f];
                pk_game_step(&game,pk_random_action(game.masks[0],&action_rng),pk_random_action(game.masks[1],&action_rng));
            }
        }
        write(input, inputs);
        grad_logits = {.shape = {B, T, PK_ACTIONS}};
        grad_value = {.shape = {B, T}};
        ok(cudaMalloc((void**)&grad_logits.data, B * T * PK_ACTIONS * sizeof(float)));
        ok(cudaMalloc((void**)&grad_value.data, B * T * sizeof(float)));
        std::vector<float> gl(B * T * PK_ACTIONS), gv(B * T);
        cotangent.resize(B * T * O);
        for (int r = 0; r < B * T; r++) {
            for (int o = 0; o < PK_ACTIONS; o++) {
                float g = inputs[r*PK_OBS+480+o] ? 0.001f + 0.0007f * sinf((r * 11 + o) * 0.029f) : 0;
                gl[r * PK_ACTIONS + o] = cotangent[r * O + o] = g;
            }
            gv[r] = cotangent[r * O + PK_ACTIONS] = 0.2f;
        }
...
static void forward_parity(Test& t) {
    t.loss();
    auto train_output = read(((PKDecoderActs*)t.train.decoder)->out);
    size_t floats = t.params.total_bytes / sizeof(float);
    assert(floats == pk_parameter_count(Test::H, Test::L));
    ...
    PKCpuPolicy* cpu = pk_cpu_make(&weights, Test::B, Test::H, Test::L);
    std::vector<float> batch(Test::B * PK_OBS);
    float max_cpu = 0, max_train = 0;
    for (int step = 0; step < Test::T; step++) {
        for (int b = 0; b < Test::B; b++) {
            std::memcpy(batch.data() + b * PK_OBS,
                t.inputs.data() + (b * Test::T + step) * PK_OBS, PK_OBS * sizeof(float));
        ...
        float* reference = pk_cpu_forward(cpu, batch.data());
    ...
    pk_cpu_free(cpu);
    printf("Pokemon semantic policy forward: CPU/rollout max=%g train/rollout max=%g (including terminal reset)\n", max_cpu, max_train);
}
...
static void gradients(Test& t) {
    ...
    for (int p = 0; p < t.params.num_regs; p++) {
        ...
        int largest=0;
        for(int i=1;i<n;i++)if(fabsf(gradients[p][i])>fabsf(gradients[p][largest]))largest=i;
        int indices[4] = {0, n / 2, cols - 1, largest};
    ...
    printf("Pokemon semantic policy BPTT: %d matrices, sampled finite differences, max error=%g; update loss %.8g -> %.8g PASS\n",
        t.params.num_regs, worst, before, after);
}
int main() {
    Test test; forward_parity(test); gradients(test);
    ok(cudaDeviceSynchronize()); puts("Pokemon semantic policy CPU/CUDA/gradient tests PASS");
}
